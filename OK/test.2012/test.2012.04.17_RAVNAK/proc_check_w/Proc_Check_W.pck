CREATE OR REPLACE Package Proc_Check_W AUTHID CURRENT_USER
is
  Version constant char(12) := '#~15032012~#';
...
/
CREATE OR REPLACE Package Body Proc_Check_W is
  Version constant char(12) := '#~15032012~#';
--------------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    ssum:=sopf+svsbs+sbranch+sreg+sdis+scod;
  ...
  loop
    ...
    loop
      ...
      then
        ...
        if cnt<15
        then
          err('Ошибка для отчетности ИИАС,Отдел Кадров,Сведения о работе-Мехнат Фаолияти:Таб.Номер='||rec_ok.TABNO||' Разница даты начала работы в должности и даты рождения сотрудника не должна быть меньше 15 лет '||to_char(rec_okper.in_office_date));
        end if;
      end if;
...
/*  for Rec in (select a.account,b.cnt from ld_account a,(select branch,account,count(account) cnt from ld_account where id in(select id from ld_forms where state=2) group by branch,account) b
               where a.branch=info.getBranch and a.acc_type_id in (2,4,31,33) and b.branch=a.branch and b.account=a.account and b.cnt>1 order by 1)
  loop
    Err('Ошибка в подсистеме Кредиты: Счет '||rec.account||' используется в нескольких анкетах');
  end loop;
*/
...
--   for rec in (select /*+ INDEX (a XK3_ACCOUNT) */ a.id, abs(a.s_out/100) s from account a where a.branch=info.getBranch  and a.acc_bal in(select bal from ss_ld_acc_bal where acc_type in(2,4,31,33))
--                 and a.s_out<>0 and a.id not in (select /*+ INDEX (ld_account IND_LD_ACCOUNT) */ account from ld_account where branch=a.branch and id in (select id from ld_forms where branch=a.branch and ld_type=1)  and account is not null)
--                 and a.id not in (select /*+ INDEX (ld_account IND_LD_ACCOUNT) */ account from ld_account where branch=a.branch and id in (select id from ld_forms where branch=a.branch)  and account is not null and proc_ld.getaccbalbytype(id,acc_type_id) = substr(account,1,5)))
--   loop
--     Err('Ошибка в подсистеме Кредиты: Счет '||rec.id||' с остатком '||rec.s||' не указан ни в одной анкете кредита');
--   end loop;
   ...
   if info.GetBankType = '004' then  -- Проверки Агробанка
    ...
    -- Письмо Агро банка №02-5-03/40 от 01.02.2011
    for rec in (select f.branch,f.id,'Остаток счета ссудной задолженности' name,abs(s1.s_out/100) sum1  from ld_forms f, (select s.branch,s.id,s.s_out from ld_saldo s where s.acc_id=2 and s.kind=0 and s.v_date=(select /*+ INDEX_DESC (ld_saldo XPK_LD_SALDO)*/ v_date from ld_saldo
                where branch=s.branch and id=s.id and acc_id=s.acc_id and kind=s.kind and rownum=1)) s1 where (f.branch,f.use_id) in (select a.branch,a.id from ld_forms a,ld_char b where a.branch=info.getBranch and a.state=2 and b.branch=a.branch and b.id=a.id and b.status=4) and f.state=2
                and s1.branch=f.branch and s1.id=f.id and s1.s_out<>0
                union all
                select f.branch,f.id,'Остаток счета просроченной задолженности' name,abs(s1.s_out/100) sum1  from ld_forms f,(select s.branch,s.id,s.s_out from ld_saldo s where s.acc_id=3 and s.kind=0 and s.v_date=(select /*+ INDEX_DESC (ld_saldo XPK_LD_SALDO)*/ v_date from ld_saldo
                where branch=s.branch and id=s.id and acc_id=s.acc_id and kind=s.kind and rownum=1)) s1 where (f.branch,f.use_id) in (select a.branch,a.id from ld_forms a,ld_char b where a.branch=info.getBranch and a.state=2 and b.branch=a.branch and b.id=a.id and b.status=4) and f.state=2
                and s1.branch=f.branch and s1.id=f.id and s1.s_out<>0 order by 1,2,3)
    loop
      Err('Ошибка в подсистеме Кредиты: В анкете кредита '||rec.id||' '||rec.name||'  не перенесен в анкету судебного разбирательства!');
    end loop;
   end if;
end;
--------------------------------------------------------------------------------
...
-- Проверки денежных переводов
 ...
 For Rec In (Select s.name_o, Sum_DP-Sum_ABS Razn from DPF_Rep, S_VEOPER s where branch=info.GetBranch and v_date = vtmpDat and s.kod_o=veoper)
 Loop
   if Rec.Razn < 0 then
 ...
 For Rec In (select ac.acc_bal, -sum(ac.s_out)/100 summa from account ac where ac.branch=info.GetBranch and ac.state=2 and ac.acc_bal in (select ab.acc_bal from ss_os_acc_bal ab where ab.id_type=1)
              group by ac.acc_bal
              minus
             select ab.acc_bal, sum(decode( sign(nvl(Pkg_Os.GetKoeff(rs.branch,rs.id_res,info.Getday),-1)), -1, 0, decode( nvl(sign(rs.EDATE-info.Getday),1), -1, 0, decode( nvl(sign(info.Getday-rs.bdate),-1), -1, 0, Pkg_Os.GetStoim(rs.branch,rs.id_res,info.Getday) )))) sum_get_stoim
               from ss_os_acc_bal ab, os_res rs where ab.id_type=1 and rs.id_grp(+)=ab.id_grp and rs.branch(+)=info.GetBranch group by ab.acc_bal)
 Loop
 ...
 if to_char(info.GetDay,'dd') > '24' then
   select count(*) Into cnt from os_res rs where rs.branch=info.GetBranch and pkg_os.GetStatus(rs.branch, rs.id_res, info.GetDay) in (1,3) and rs.koeff>0
      ...
      and (rs.branch, rs.id_res) not in (select ra.branch, ra.id_res from os_resamort ra where ra.branch=info.GetBranch and ra.date_from=trunc(info.GetDay,'mm'));
   if cnt<>0 then
--      Err('Ошибка в подсистеме ОC: В модуле Учет движения ОС не начислена амортизация за текущий месяц. Cмотрите Дату последней амортизации в Отчете Ведомость  наличия ОС , полученном в Excel');
   ...
   for rec in (
    select rs.invnom, rs.name_res from os_res rs
    where rs.branch=info.GetBranch and pkg_os.GetStatus(rs.branch, rs.id_res, info.GetDay) in (1,3) and rs.koeff>0
    and (rs.edate is null or rs.edate>info.getday) and rs.damort<info.getday and Pkg_Os.GetStoim(rs.branch, rs.id_res, info.GetDay)!=Pkg_Os.GetIznos(rs.branch, rs.id_res, info.GetDay)
    and (rs.branch, rs.id_res) not in
      (select ra.branch, ra.id_res from os_resamort ra
      where ra.branch=info.GetBranch and ra.date_from=trunc(info.GetDay,'mm')))
   loop
   ...
   for rec in (select rs.invnom, rs.id_grp, rs.branch, rs.id_res from os_res rs where rs.branch=info.GetBranch and pkg_os.GetStatus(rs.branch, rs.id_res, info.GetDay) in (1,3) and rs.koeff>0
               and (rs.edate is null or rs.edate>info.getday) and rs.damort<info.getday and Pkg_Os.GetStoim(rs.branch, rs.id_res, info.GetDay)!=Pkg_Os.GetIznos(rs.branch, rs.id_res, info.GetDay) )
   loop
     ...
     Begin
       Select /*+ index_desc(ss_os_norm xpk_ss_os_norm)*/
              proc,date_from into vtmpkoeff2, vtmpDat
         from ss_os_norm
        where id_grp=vtmpid_grp
          and date_from <= last_day(info.GetDay)
          and rownum = 1;
     Exception
...
-- писмо ХЛБ № 07-Б-29/189 от 22.04.2011
   for rec in (select ac.acc_bal, -sum(ac.s_out)/100 ss from account ac where ac.branch=ac.branch and ac.state=2 and ac.acc_bal in (select distinct substr(ACC_SKLAD,1,5) from ss_buh_grp) group by ac.acc_bal
      minus    select '19921' acc_bal, nvl(sum("Сумма"),0) ss from (select * from (select substr(mbp.getNameTmc(m.id_tmc),1,80) "Наименование",  max(n.DATE_NKL) "Дата",
                        substr(mbp.getNameEdi(m.id_edi),1,80) "Ед.изм.", sum(m.kolvo) "Количество", m.stoim "Цена", sum(m.ammount) "Сумма", substr(mbp.getNameGrp(m.id_grp),1,80) name_grp,m.id_grp, m.id_tmc, m.id_edi
                        from buh_mbp m , buh_nkl n where m.branch =n.branch and m.id_nkl =n.id_nkl and n.state in (0,1,3)
                group by m.id_grp, m.id_tmc, m.id_edi, m.stoim) where "Количество" > 0))
   Loop
   ...
   if info.GetRegionId in ('03','10','12','22','26','33','35','14','18','06','08') then
     for rec in (select nvl(sum(r.in_stoim),0) from os_nkl n, os_res r where n.id_oper in (1, 62, 64) and n.branch = info.GetBranch and n.date_nkl <= info.GetDay and r.id_nkl = n.id_nkl and r.branch = n.branch
      and nvl(pkg_os.GetKoeff(r.branch,r.id_res,info.GetDay),0)=0
       and pkg_os.GetStatus(r.branch,r.id_res,info.GetDay)=1
       minus  select -sum(ac.s_out)/100 from account ac where ac.branch=info.GetBranch and ac.state=2 and ac.acc_bal in (select ab.acc_bal from ss_os_acc_bal ab where ab.id_type=4))
     Loop
...
-- письмо №07-Б-25/721 от 29.11.2011
  if info.GetRegionId in ('26','30') then
    for rec in (select ab.acc_bal, round( sum ( nvl(s.koeffpereoc,1)*Pkg_Os.GetIznos(r.branch, r.id_res, info.getday) ), -1 ) summa
         from ss_os_acc_bal ab, os_res r, os_resstoim s where ab.id_type=2 and r.branch=info.getbranch and Pkg_Os.GetKoeff(r.branch, r.id_res, info.getday) >=0
          and (r.EDATE is null or r.EDATE > info.getday) and r.bdate < info.getday and Pkg_Os_rep.GET_ID_GRP(r.branch, r.id_res, info.getday) = ab.id_grp
          and s.branch(+)=r.branch and s.id_res(+)=r.id_res and s.date_from(+)=trunc(to_date(info.getday),'mm') and s.pereoc(+)=1 group by ab.acc_bal
       minus select ac.acc_bal, round(sum(ac.s_out)/100 , -1) summa from account ac where ac.branch=info.getbranch and ac.state=2
          and ac.acc_bal in (select ab.acc_bal from ss_os_acc_bal ab where ab.id_type=2) group by ac.acc_bal)
    loop
      dm('Ошибка в подсистеме Основные средства: Для балансового счета : '|| rec.acc_bal ||' остаток( '|| to_char(rec.summa) ||' ) есть расхождение с модулем Счета!');
    end loop;
  end if;
end;
--------------------------------------------------------------------------------
