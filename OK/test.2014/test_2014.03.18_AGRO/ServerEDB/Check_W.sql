...
is
  Version constant char(12) := '#~19032014~#';
  ...
  if nvl(to_number(const.GetConst(24003)),32)<=to_number(to_char(info.GetDay,'dd')) then
    if nvl(to_number(const.GetConst(24003)),32)<=to_number(to_char(info.GetDay,'dd')) then
      for rec in( select count(*) num, branch, a.service_id from cs_contract a where branch=info.GetBranch having count(*)>0 group by branch, a.service_id) loop
        select count(*) into i from cs_date a where branch = info.getbranch and to_char(d_date,'mmyyyy') = to_char(add_months(info.GetDay,-1),'mmyyyy') and state<>1 and a.branch=rec.branch and a.service_id=rec.service_id;
        if i>0 then
          err('"Комиссионные услуги" модулида утган ой учун абонент тулови олинмаган!');
        end if;
      end loop;
    end if;
  end if;
...
-- Проверки денежных переводов
 ...
 For Rec In (select ac.acc_bal, -sum(ac.s_out)/100 summa from account ac where ac.branch=info.GetBranch and ac.state=2 and ac.acc_bal in (select ab.acc_bal from ss_os_acc_bal_ ab where ab.id_type=1) group by ac.acc_bal
              minus
             select ab.acc_bal, sum(nvl(stoim_,0)) sum_get_stoim from ss_os_acc_bal_ ab,(select /*+ index (r XPKOS_RES)*/ r.branch, r.ID_RES, Pkg_Os_rep.GET_ID_GRP(r.branch, r.id_res, info.GetDay) id_grp_, Pkg_Os_rep.GET_ID_subGRP(r.branch, r.id_res, info.GetDay) id_subgrp_, Pkg_Os.GetStoim(r.branch, r.id_res, info.GetDay) stoim_
               from os_res r, ss_os_grp sog, ss_os_subgrp sss where Pkg_Os_rep.GET_ID_GRP(r.branch, r.id_res, info.GetDay) = sog.id_grp and Pkg_Os_rep.GET_ID_SUBGRP(r.branch, r.id_res, info.GetDay) = sss.id_subgrp and r.branch=info.GetBranch and r.rab=3 and (Pkg_Os.GetKoeff(r.branch, r.id_res, info.GetDay)>=0 or nvl(Pkg_Os.GetIdVid(r.branch, r.id_nkl),0)=9)
...
-- писмо ХЛБ № 07-Б-29/189 от 22.04.2011
   ...
   for rec in (select nvl(sum(r.in_stoim),0) from os_nkl n, os_res r where n.id_oper in (1, 62, 64)  and n.branch = info.GetBranch and n.date_nkl <= info.GetDay and r.id_nkl = n.id_nkl and r.branch = n.branch and r.rab=3 and nvl(pkg_os.GetKoeff(r.branch,r.id_res,info.GetDay),0)=0 and nvl(n.id_vid,0)!=9 and pkg_os.GetStatus(r.branch,r.id_res,info.GetDay)=1
