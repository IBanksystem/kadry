...
// END добавление смотрящих-выше-полей
  ...
  if (HeadBranch_ <>'09006') and (HeadBranch_ <>'09004')  // 08.11.2012 теперь и в агро подтверждение ГО
  then begin
...
/////////////////
        ...
        then ErMessage := ErMessage+' "Укажите * Код района, подразделением УВД которого выдан паспорт" ';
...
//          showmessage('Если указан реквизит «Наличие нострификации» как «1» (есть), то реквизиты «Номер и дата нострификации» должны быть заполнены обязательно');
        ...
        try
          ...
          or (raznica < 15)
          then begin
            er_mes:=er_mes+'(05958)Разница даты поступления и даты рождения должна быть больше 15 лет и меньше возраста сотрудника'+#13;
          end;
        except
        ...
        try
          ...
          then begin
            er_mes:=er_mes+'(05960)Разница даты окончания и даты поступления в учебное заведение не должна быть больше 20 лет и меньше 1 года'+#13;
          end;
        except
            er_mes:=er_mes+'Проверьте дату поступления '+cdsOKEducation.FieldByName('begin_date_dd').AsString+'.'+
          cdsOKEducation.FieldByName('begin_date_mm').AsString+'.'+
...
// проживания
       ...
       then begin
         ...
         then begin
           er_mes:=er_mes+'Если год смерти не указан, то число и месяц смерти не должны указываться'+#13;
         end;
       end;
...
////////////////////////
    ...
    then begin
      ...
      then begin
        ...
        showmessage('(9074)Укажите категорию филиала '+branch+' в халк банке.(заполните Справочник Категорий Филиалов)');
      end else begin
...
////////////////////////
      ...
      then begin
        if errMes<>''
        then begin
          showmessage('(9016)Должность (код '+cdsOKStat.fieldbyname('post_code').asstring+') не найдена либо ее нельзя использовать в филиале(уровень '+BossLevel+')'+
               #13#13+'также укажите '+#13#13+ErrMes);
          exit;
        end else begin
          showmessage('(9020)Должность (код '+cdsOKStat.fieldbyname('post_code').asstring+') не найдена либо ее нельзя использовать в филиале(уровень '+BossLevel+')');
          exit;
        end;
      end;
...
//  if count_>0
//  then begin
//    showmessage('Нельзя удалить должность из штатного расписания, если эта должность занята.');
//  end else begin
...
//      cdsOKStat.Delete;
  //    cdsOK_AfterPost(dsOKStat.DataSet);
      try
        ...
        do begin
          execsql('rollback');
          showmessage('Не удалось удалить'+#13+e.Message);
          cdsOKStat.refresh;
        end;
      end;
    End;
...
//      TData(CBDep.Items.Objects[CBDep.ItemIndex]).Id;
      ...
      then begin
        ...
        then begin
          ...
          then begin
            mfo_category_type_code :='0';
            showmessage('(13489)Укажите категорию филиала '+branch+' в халк банке.(заполните Справочник Категорий Филиалов)');
          end else begin
            mfo_category_type_code :=vartostr(mfo_category_type_code_VAR) ;
          end;
...
//  nn:=cdsTempSource.FieldCount-1;
  ...
  inc(row);
  i:=1;
  V.Cells[row, i ].Value := 'должностей "специализация"';
...
//  and (cdsOKPersonal.FieldByName('GOsubmit_code').AsString<>'')
  then begin
    showmessage('Нельзя отправить заявку, сотрудник в состоянии '+dblcbGOsubmit.text);
    exit;
  end;
  ...
  end;
{
  ...
  if (doljn_counter_=0) and
    (cdsOKPersonal.fieldbyname('POST_CODE').asstring<>'')
  then begin
    // не был и не назначен на руководящую должность
      actAppointExecute(nil);
  end else begin
    // был либо назначен на руководящую должность
    // ИЛИ принимается на работу в первый раз либо при возвращении после увольнения
}
     //        \\
    // проверка \\
   //            \\
    try
      ...
      then begin
        ...
        if doljn_counter_=0 then ErMessage:=ErMessage+' отсутствует фото ';
        // наличие образования
        ...
        if doljn_counter_=0 then ErMessage:=ErMessage+' отсутствует образование в доп.сведениях ';
      ...
      ////
          ...
          then begin
            ...
            try
              ...
              showmessage('Не удалось открыть в блокноте "notepad C:\NCI Bank\OK\Temp\err_ok3.txt"');
            end;
          end;
      ////
...
//      except on e:exception
//        do begin
//          showmessage('(21540)Ошибка '+#13+e.Message);
//        end;
//      end;
    ...
    with TOpenDialog.Create(Self) do
     try
       ...
       begin
         ...
         and (copy(FileName,Length(FileName)-3,4) <>'.JPG')
         then begin
           showmessage('Требуется файл формата .pdf или .jpg а не '+copy(FileName,Length(FileName)-3,4));
         end else begin
...
//    cdsOKPERSONAL.FieldByName('special_code').AsString:='';
  ...
  then begin
    ...
    then begin
      if  ( cdsOKPersonal.FieldByName('cod_distr_uvd').AsString='')
      then  ErMessage := ErMessage+' "* Код района, подразделением УВД которого выдан паспорт" ';
    end else begin
      if  ( cdsOKPersonal.FieldByName('cod_distr_prim').AsString='')
      then  ErMessage := ErMessage+' "* Примечание к Код района, подразделением УВД которого выдан паспорт" ';
    end;
  end;
...
//  then begin
//    if  ( cdsOKPersonal.FieldByName('PASS_REG').AsString='')
//    then begin то ставь то убери - идиоты))
//      ErMessage := ErMessage+' " * Место Регистрации" ';
//    end;
//  end;
...
// переделать на пробег по всем записям в доп.таблицах о сотруднике
   ...
   if z=0
   then ErMessage  :=ErMessage  +'(26115)отсутствует фото'+#13;
...
// ... знает почему не выходит сообщение о штатном расписании
   ...
   then begin
     if z=0
     then begin
       ErMessage  :=ErMessage  +'(26130)Указанное рабочее место не найдено в штатном расписании'+#13;
     end else begin
       z2:= getsqlresultpar(
         ' select count(*)'+
         ' from ok_personal'+
       ...
       if z2>=z
       then ErMessage  :=ErMessage  +'(26143)В штатном расписании указано'+inttostr(z)+'рабочих мест. Фактически назначено'+inttostr(z2)+#13;
     end;
   end;
     Tabokeducation_0Show(nil);

   if cdsOKEducation.recordcount=0
   then ErMessage  :=ErMessage  +'отсутствуют сведения об образовании'+#13;
    ...
    then begin
      ...
      then begin
        ...
        if z=0
        then ErMessage  :=ErMessage  +'отсутствует второе образование(* Наличие второй специальности)'+#13;
      end;
    end;
...
//showmessage('cdsOKEducation.FieldByName(''END_DATE'')='+cdsOKEducation.FieldByName('END_DATE').asstring);
       ...
       then begin
         ...
         if z=0
         then er_mes:=er_mes+' отсутствует копия диплома'+#13;
       end;
...
//  1: If (cdsOKacademic.State in dsEditModes) and (cdbgOKacademic.CanFocus)
     ...
     cdsOKacademic.first;
       ...
       then begin
//         showmessag e(
         er_mes := er_mes+'В Сведениях для ИИАС указано "Ученое звание Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         er_mes := er_mes+'В Сведениях для ИИАС указано "Ученого звания Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Ученое звание'+#13;
  //       dblcbSSOKAcademic.SetFocus;
    //     exit;
       end else begin
         z:=getsqlresultpar('select count(*) from s_obraz so ,ss_ok_nci son where son.nci_id=:p_1 and so.OBR_ID=son.s_id and son.ok_id=:p_2',
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Реквизит Ученое звание должен соответствовать справочнику соответствия.'+#13;
  //          dblcbSSOKAcademic.SetFocus;
    //        exit;
         end;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Дата присвоения (ученого звания)'+#13;
  //       dbedAcademicDate.SetFocus;
    //     exit;
       end;
...
//  2: If (cdsOKDegree.State in dsEditModes) and (cdbgOKDegree.CanFocus)
     ...
     cdsOKDegree.first;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Ученая степень Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Ученой степени Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Ученая степень'+#13;
  //       dblcbSSOKdegree.SetFocus;
    //     exit;
       end else begin
         z:=getsqlresultpar('select count(*) from s_obraz so ,ss_ok_nci son where son.nci_id=:p_1 and so.OBR_ID=son.s_id and son.ok_id=:p_2',
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Реквизит Ученая степень должен соответствовать справочнику соответствия.'+#13;
  //          dblcbSSOKdegree.SetFocus;
    //        exit;
         end;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Дата присвоения (ученой степени)'+#13;
  //       dbedDegreeDate.SetFocus;
    //     exit;
       end;
       ...
       try
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Дата присвоения (ученой степени)='+cdsOKDegree.FieldByName('DEGREE_DATE').AsString;
  //         dbedDegreeDate.SetFocus;
    //       exit;
         end;
       except on e:exception
...
//  3: If (cdsOKaward.State in dsEditModes) and (cdbgOKaward.CanFocus)
     ...
     cdsOKaward.first;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Награды Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Наград Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Сведения о награде'+#13;
  //       dbedAwardInfo.SetFocus;
    //     exit;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Год получения (награды)'+#13;
  //       dbedAwardDate.SetFocus;
    //     exit;
       end;
//
...
//     Then begin
     ...
     cdsOKScientific.first;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Научные работы Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Научных работ Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Сведения о научной работе'+#13;
  //       dbedScientificInfo.SetFocus;
    //     exit;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Год написания (научной работы)'+#13;
  //       dbedScientificDate.SetFocus;
    //     exit;
       end;
//
...
//     Then begin
     TabSheet_5Show(nil);
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Зарубежные поездки Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Зарубежных поездок Нет"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Страна по справочнику ЦБ'+#13;
  //       DBEdit3.SetFocus;
    //     exit;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'(05975)Реквизит «Пребывание за границей страна» не должен быть равен значению «Узбекистан»'+#13;
  //       exit;
       end;
//
       if cdsOKVoyage.FieldByName('VOYAGE_INFO').AsString=''
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Сведения о зарубежной поездке'+#13;
  //       DBEdit3.SetFocus;
    //     exit;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Год зарубежной поездки'+#13;
  //       DBEdit1.SetFocus;
    //     exit;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Разница даты выезда за границу и даты рождения сотрудника не должна быть меньше 18  и больше возраста сотрудника'+#13;
  //       DBEdit1.SetFocus;
    //     exit;
       end;
  ...
  //       DBEdit1.SetFocus;
    //     exit;
       ...
       then begin
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Дата возвращения из-за границы не должна быть меньше даты выезда  и не должна быть больше текущей даты'+#13;
  //         exit;
         end;
       end;
...
//     Then begin
     ...
     cdsOKLanguage.first;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Ин.яз. Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Ин.яз. Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Иностранный язык'+#13;
  //       exit;
       end else begin
                z:=getsqlresultpar('select count(*) from s_langs so ,ss_ok_nci son where son.nci_id=:p_1 and so.LANG_ID=son.s_id and son.ok_id=:p_2',
                 '77',cdsOKLanguage.FieldByName('language_code').AsString
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Реквизит Иностранный язык должен соответствовать справочнику соответствия.'+#13;
  //          exit;
         end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Уровень владения (иностранным языком)'+#13;
  //       DBLCBlanglevel.SetFocus;
    //     exit;
       end else begin
                z:=getsqlresultpar('select count(*) from s_lang so ,ss_ok_nci son where son.nci_id=:p_1 and so.UROV_ID=son.s_id and son.ok_id=:p_2',
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Реквизит Уровень владения (иностранным языком) должен соответствовать справочнику соответствия.'+#13;
  //          exit;
         end;
...
*)
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Фамилия (родственник)'+#13;
  //       DBEdit701.SetFocus;
    //     exit;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Имя (родственник)'+#13;
  ///       DBEdit702.SetFocus;
     //    exit;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Родственное отношение (родственник)'+#13;
  //       exit;
       end else begin
         // взять код типа родственника из справочника соответствия
         S_ID:= getsqlresultpar('select s_id from ss_ok_nci where nci_id=:p_1 and ok_id=:p_2','81',cdsOKRelation.FieldByName('RELATION_CODE').AsString);
         ...
         then begin
           ...
           then begin
//             showmessag e(
         ErMessage := ErMessage+'Если пол сотрудника указан – «Мужской», то код степени родства не может принимать значение «Муж»'+#13;
  //           exit;
           end;
         end
          // пол женский родственник жена ошибка
           ...
           then begin
  //           showmessag e(
         ErMessage := ErMessage+'Если пол сотрудника указан – «Женский», то код степени родства не может принимать значение «Жена»'+#13;
//             exit;
           end;
         end;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Год рождения (родственник)'+#13;
  //       exit;
       end else begin
         if (cdsOKRelation.FieldByName('RELATION_BIRTHDAY').AsInteger <
               strtoint(copy( cdsOKpersonal.FieldByName('BIRTHDAY').asstring ,7,4)))
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Если родственник логически указан как младшее поколение, то год рождения родственника должен быть больше года рождения сотрудника'+#13;
  //         exit;
         end;
         if (  cdsOKRelation.FieldByName('RELATION_BIRTHDAY').AsInteger >
                 strtoint( copy( cdsOKpersonal.FieldByName('BIRTHDAY').asstring ,7,4))   )
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Если родственник указан как старшее поколение, то год рождения родственника должен быть меньше года рождения сотрудника'+#13;
  //         exit;
         end;
         if cdsOKRelation.FieldByName('RELATION_BIRTHDAY').AsInteger<1900
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Проверьте год рождения '+
             cdsOKRelation.FieldByName('RELATION_BIRTHDAY').AsString;
  //         exit;
         end;
...
// проживания
       ...
       then begin
         ...
         then begin
  //         showmessag e(
         ErMessage := ErMessage+'Проверьте дату рождения '+cdsOKRelation.FieldByName('RELATION_BIRTHDAY').AsString+
           ' и дату сметри '+
            cdsOKRelation.FieldByName('RELATION_DEATHDAY').AsString;
//           exit;
         end;
...
//then MemoTest.lines.add('*33*');
   ...
   then ErMessage  :=ErMessage  +'отсутствуют сведения о работе'+#13;
...
//
       ...
       then begin
       // если шу банкда и код банка пусто то поставить туда текущее мфо
         ErMessage := ErMessage+'если шу банкда и код банка пусто то поставьте туда текущее мфо'+#13;
//          cdsOKPeriod.FieldByName('COD_BANK').AsString:=branch;
       end;
//
       if cdsOKPeriod.FieldByName('OFFICE_NAME').AsString=''
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Наименование организации'+#13;
  //       DBEdit801.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Адрес организации'+#13;
  //       DBEdit802.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Занимаемая должность'+#13;
  //       DBEd_poststring.SetFocus;
    //     exit;
       end;
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Дата поступления'+#13;
  //       exit;
       end else begin
         if (cdsOKPeriod.FieldByName('IN_OFFICE_DATE').AsDateTime<cdsOKPersonal.FieldByName('birthday').AsDateTime)
         then begin
         ...
         or (raznica<15)
         then begin
           er_mes:=er_mes+'Разница даты начала работы в должности и даты рождения сотрудника не должна быть меньше 15 лет и не должна быть больше возраста сотрудника'+#13;
         end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Дата приказа/основания'+#13;
  //       exit;
       end else begin
         if cdsOKPeriod.FieldByName('IN_OFFICE_DATE').AsString<>''
         then begin
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Код типа стажа'+#13;
  //       DBLcbTypePeriod.SetFocus;
    //     exit;
       end else begin
         if (cdsOKPeriod.FieldByName('TYPE_PERIOD_CODE').AsString='1')
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Номер приказа/основания'+#13;
  //       DBEdit811.SetFocus;
    //     exit;
       end;
//
...
//     Then begin
     ...
     cdsOKRise.first;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Квалификация Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Квалификация Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Сведения о повышении (квалификации)'+#13;
  //       DBEdit12.SetFocus;
    //     exit;
       end;
//
...
//         showmessag e(
  ...
  //       DBEdit46.SetFocus;
    //     exit;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Год прохождения (квалификации)'+#13;
  //       DBEdit46.SetFocus;
    //     exit;
       end else begin
         if (cdsOKRise.FieldByName('rise_date_year').AsInteger> strtoint(copy(bankdate,7,4))  )
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Разница даты повышения квалификации и даты рождения сотрудника не должна быть меньше 18  и больше возраста сотрудника'+#13;
  //         exit;
         end;
         if  (cdsOKRise.FieldByName('rise_date_year').AsString<>'')
         and (cdsOKRise.FieldByName('rise_date_mm').AsString<>'')
...
//     Then begin
     ...
     cdsOKElection.first;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Избрание Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Избрание Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Должность'+#13;
  //       DBEdit48.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Начало(год)'+#13;
  //       DBEdit47.SetFocus;
    //     exit;
       end else begin
         if (cdsOKElection.FieldByName('election_date_begin').AsInteger> strtoint(copy(bankdate,7,4))  )
         ...
         then begin
//           showmessag e(
         ErMessage := ErMessage+'Разница даты избрания и даты рождения сотрудника не должна быть меньше 18  и больше возраста сотрудника'+#13;
  //         exit;
         end;
         if  (cdsOKElection.FieldByName('election_date_begin').AsString<>'')
         and (cdsOKElection.FieldByName('election_date_begin_mm').AsString<>'')
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Завершение(год)'+#13;
  //       DBEdit14.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
    //     showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Исполнительный орган'+#13;
//         DBLCBelection.SetFocus;
  //       exit;
       end;
//
...
//     Then begin
     TabSheet_11Show(nil);
       ...
       then begin
  //       showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Гос премии Есть"'+#13;
//         exit;
       end;
...
//
       ...
       then begin
  //       showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Гос премии Нет"'+#13;
//         exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Наименование'+#13;
  //       dbedit15.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
    //     showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Год получения'+#13;
//         dbedit16.SetFocus;
  //       exit;
       end;
//
...
//     Then begin
     TabSheet_12Show(nil);
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Армия Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Армия Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
    //     showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Отношение'+#13;
//         DBLookupComboBox7.SetFocus;
  //       exit;
       end;
//
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Годность'+#13;
  //       DBLookupComboBox6.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Вид состава'+#13;
  //       DBLcbStaff.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Род войск'+#13;
  //       DBLCBtypeforce.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Группа учета'+#13;
  //       DBLCBtypeforce.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
    //     showmessag e (
         ErMessage := ErMessage+'Не заполнено поле '+'Категория учета'+#13;
//         DBLCBcategoryarmy.SetFocus;
  //       exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'В/Специальность'+#13;
  //       dbedit52.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Номер ВУС'+#13;
  //       dbedit50.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'В/Звание'+#13;
  //       DBLCBmilitaryrank.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
    //     showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Код должности и особые признаки в/службы'+#13;
//         dbedit18.SetFocus;
  //       exit;
       end;
//
//
       ...
       then begin
    //     showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Военкомат'+#13;
//         dbedit55.SetFocus;
  //       exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Спец.учет N'+#13;
  //       dbedit53.SetFocus;
    //     exit;
       end;
//
...
//     Then begin
     ...
     cdsOKParty.first;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Партия Есть"'+#13;
  //       exit;
       end;
...
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'В Сведениях для ИИАС указано "Партия Нет"'+#13;
  //       exit;
       end;
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Партия'+#13;
  //       DBLCBparty.SetFocus;
    //     exit;
       end;
//
...
//
       ...
       // беспартийный
           ...
           then begin
    //         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Год вступления в партию'+#13;
//             dbedit73.SetFocus;
  //           exit;
           end;
    //
           ...
           then begin
    //         showmessag e(
         ErMessage := ErMessage+'Неправильный='+cdsOKParty.FieldByName('party_date').AsString+' Год вступления в партию'+#13;
//             dbedit73.SetFocus;
  //           exit;
           end;
    //
    //
           ...
           then begin
             ...
             then begin
    //           showmessag e(
         ErMessage := ErMessage+'Вы ввели Причину партийного наказания,'+#13+
                 cdsOKParty.FieldByName('punishment_motive').AsString+#13+
                 'но Не заполнили поле '+#13+
...
//               dbed_gpn.SetFocus;
  //             exit;
           ...
           then begin
             ...
             then begin
    //           showmessag e(
         ErMessage := ErMessage+'Вы ввели Год парт.наказания,'+#13+
                 cdsOKParty.FieldByName('punishment_date').AsString+#13+
                 'но Не заполнили поле '+#13+
  ...
  //             exit;
    ...
    //
           ...
           then begin
             ...
             then begin
    //           showmessag e(
         ErMessage := ErMessage+'Вы ввели Причину выхода из партии,'+#13+
                 cdsOKParty.FieldByName('out_party').AsString+#13+
                 'но Не заполнили поле '+#13+
  ...
  //             exit;
    ...
    //
           ...
           then begin
             ...
             then begin
    //           showmessag e(
         ErMessage := ErMessage+'Вы ввели Год выхода из партии,'+#13+
                 cdsOKParty.FieldByName('party_outdate').ASstring +#13+
                 'но Не заполнили поле '+#13+
...
//
       ...
       then begin
    //     showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Вид отпуска'+#13;
//         DBLookupComboBox13.SetFocus;
  //       exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Кол-во дней отпуска'+#13;
  //       dbedit63.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Номер приказа'+#13;
  //       dbedit62.SetFocus;
    //     exit;
       end;
//
       ...
       then begin
    //     showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Рабочий год- начало'+#13;
//         dbedit60.SetFocus;
  //       exit;
       end;
//
//
       ...
       then begin
//         showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Р/ год- окончание'+#13;
  //       dbedit61.SetFocus;
    //     exit;
       end;
//
//
       ...
       then begin
           ...
           then begin
//             showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Дата выхода в отпуск'+#13;
  //           DBEDdate_in_leave.SetFocus;
    //         exit;
           end;
           if cdsOKLeave.FieldByName('date_out_leave').AsString=''
           then begin
//             showmessag e(
         ErMessage := ErMessage+'Не заполнено поле '+'Дата выхода из отпуска'+#13;
  //           dbedit26.SetFocus;
    //         exit;
           end;
       end;
...
////
    ...
    if num_=0 then showmessage('Готово.');
  end;
     ...
     try
       ...
       begin
         ...
         then begin
          ...
          CreateParam(ftString,'p_4' ,ptInput);
          ParamByName('p_4').Value :='4'; // 4 справка махалля
            end;
...
begin
      ...
      do begin
          ...
          ParamByName('p_3').Value := cdsOKAddInform.FieldByName('personal_code').AsString;
          CreateParam(ftString,'p_4' ,ptInput);
          ParamByName('p_4').Value :='4'; // 4-справка махалла
      end;
...
begin
// селект из ss_dblink_branch и затем селект из ok_personal нужного филиала
end;

procedure TBOKFormSTAT.BitBtn173Click(Sender: TObject);
begin
// скопировать но с текущим мфо и другими id и без таб.номера
end;
...
// alter session
  ...
  then begin
    ...
    if filial_schema=''
    then begin
      showmessage('не найдено мфо "'+filial_branch+'" в таблице ss_dblink_branch');
    end else begin
     ...
     try
      ...
      then begin
        ...
        then begin
          mfo_category_type_code :='0';
          showmessage('(22102)Укажите категорию филиала '+branch+' в халк банке.(заполните Справочник Категорий Филиалов)');
        end else begin
          mfo_category_type_code :=vartostr(mfo_category_type_code_VAR) ;
        end;
        gbFilialStat.Caption:= '    Категория:'+mfo_category_type_code+'  - Штатное расписание - '+FilialName_;
      end else begin
        mfo_category_type_code :='-1';
        gbFilialStat.Caption:= '    Штатное расписание - Ввод  ';
      end;
...
//      cdsOKStat.Filtered:=true;
      ...
      then begin
          ...
          then begin
            mfo_category_type_code :='0';
            showmessage('(22250)Укажите категорию филиала '+branch+' в халк банке.(заполните Справочник Категорий Филиалов)');
          end else begin
            mfo_category_type_code :=vartostr(mfo_category_type_code_VAR) ;
          end;
          sql_ :='select sop.post_code, to_char(soc.salary) salary, soc.category_name '+
...
//        cdsOKStat.Filtered:=true;
      ...
      then begin
          ...
          then begin
            mfo_category_type_code :='0';
            showmessage('(22576)Укажите категорию филиала '+branch+' в халк банке.(заполните Справочник Категорий Филиалов)');
          end else begin
            mfo_category_type_code :=vartostr(mfo_category_type_code_VAR) ;
          end;
          sql_ :='select sop.post_code, to_char(soc.salary) salary, soc.category_name '+
    ...
    if not cdsSSOKSpecial_RO.Active then loadData(cdsSSokSpecial_RO, SQL_ssokSpecial_set);
      ...
      then begin
        ...
        then begin
          ...
          then begin
            mfo_category_type_code :='0';
            showmessage('(22828)Укажите категорию филиала '+branch+' в халк банке.(заполните Справочник Категорий Филиалов)');
          end else begin
            mfo_category_type_code :=vartostr(mfo_category_type_code_VAR) ;
          end;
          sql_ :='select sop.post_code, to_char(soc.salary) salary, soc.category_name '+
...
begin
      ...
      try
          sql_:=
            'select '+
             ' BRANCH "МФО",'+
             ' DEPARTMENT_CODE "Код О.", OK_SERVICE.GET_DEPARTMENTNAME(DEPARTMENT_CODE) "Отдел",'+
             ' POST_CODE "Код Д.", OK_SERVICE.GET_POSTNAME(POST_CODE) "Должность",'+
             ' SPECIAL_CODE "Код С.", OK_SERVICE.GET_SPECIALNAME(SPECIAL_CODE) "Специализация",'+
             ' QUANTITY_POST "Количество",'+
             ' Emp_name "Пользователь",'+
             ' User_name "Пoльзователь",'+
             ' to_char(INS_DATE,''dd.mm.yyyy hh24:mi:ss'') "Время"'+
            ' from ok_stat_hist'+
            ' where branch=branch'+
  ...
  loaddata(ExecPrc,sql_);
    ...
    //showmessage();
    // memo1.lines.add('количество строк '+inttostr(ExecPrc.recordcount));
    // memo1.lines.add('Начало '+TimeToStr(Time));
        for col:=0 to nn do
         Begin
...
var
 ...
 sql_,sql2_,NumFormat,DateFormat,StrFormat,olddepname,oldpost: String; // sum2_,
//  total_count,
  total_num,dep_num :integer; // сквозная нумерация
//  dep_total_count,   // количество по отделу
...
{ внизу если что
  ...
  inc(row);
    ...
    MyRange.Borders.Weight := 2;
    MyRange.Borders.ColorIndex := color_;
    V.Cells[row,  1].Value := 'Урин-лавозимларнинг бандлиги тугрисидаги';
  ...
  inc(row);
    ...
    MyRange.Borders.Weight := 2;
    MyRange.Borders.ColorIndex := color_;
      V.Cells[row, 1].Value := 'М А Ъ Л У М О Т';
  ...
  inc(row);
    ...
    MyRange.Borders.ColorIndex := color_;
      ...
      V.Cells[row, 10 ].Value := copy(bankdate,4,7)+' йил холатига';
...
//  nn:=cdsTempSource.FieldCount-1;
  ...
  // формирование шапки
   ...
   V.Cells[row, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+1, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+2, i ].Borders[9].ColorIndex := color_; //  нижняя
  ...
  inc(i);
   ...
   V.Cells[row+1, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+2, i ].Borders[9].ColorIndex := color_; //  нижняя
  V.Cells[row, i ].Value := '                              Лавозим                              ';
  inc(i);
   V.Cells[row, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+1, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+2, i ].Borders[9].ColorIndex := color_; //  нижняя
  V.Cells[row, i ].Value := ' Сони ';
  inc(i);
   V.Cells[row, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+1, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+2, i ].Borders[9].ColorIndex := color_; //  нижняя
  V.Cells[row, i ].Value := '     Маоши     ';
  inc(i);
   V.Cells[row, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+1, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+2, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row, i ].Borders[10].ColorIndex := color_; // правая
   V.Cells[row+1, i ].Borders[10].ColorIndex := color_; // правая
   V.Cells[row+2, i ].Borders[10].ColorIndex := color_; // правая
   V.Cells[row+3, i ].Borders[10].ColorIndex := color_; // правая

  V.Cells[row, i ].Value := '                                  Ф.И.Ш. ';
  inc(i);
   V.Cells[row, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+1, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+2, i ].Borders[9].ColorIndex := color_; //  нижняя
  ...
  inc(i);
   ...
   V.Cells[row+2, i ].Borders[10].ColorIndex := color_; // правая
   V.Cells[row+3, i ].Borders[10].ColorIndex := color_; // правая
  V.Cells[row, i ].Value := 'Шу банкга     ';
  inc(i);
//   V.Cells[row, i ].Borders[7].ColorIndex := color_; //  левая
//   V.Cells[row, i ].Borders[8].ColorIndex := color_; //  верхняя
//   V.Cells[row, i ].Borders[9].ColorIndex := color_; //  нижняя
//   V.Cells[row, i ].Borders[10].ColorIndex := color_; // правая
   ...
   V.Cells[row+1, i ].Borders[9].ColorIndex := color_; //  нижняя
   V.Cells[row+2, i ].Borders[9].ColorIndex := color_; //  нижняя
  V.Cells[row, i ].Value := '     Изох     ';


  inc(row);
  i:=6;
  V.Cells[row, i ].Value := 'номер';
  inc(i);
  inc(i);
  V.Cells[row, i ].Value := 'кабул';
  inc(i);
  V.Cells[row, i ].Value := 'ишга';

  inc(row);
  i:=8;
  V.Cells[row, i ].Value := 'килинган';
  inc(i);
  V.Cells[row, i ].Value := 'олинган';

  inc(row);
  i:=8;
  V.Cells[row, i ].Value := 'сана';
  inc(i);
  V.Cells[row, i ].Value := 'сана';
  ...
  do begin
    ...
    then begin
      ...
      do begin
          ...
          MyRange.Borders.Weight := 2;
          MyRange.Borders.ColorIndex := 1;
          V.Cells[row, 5 ].Borders[10].ColorIndex := color_; // правая
          V.Cells[row, 6 ].Borders[10].ColorIndex := color_; // правая
          V.Cells[row,  7].VerticalAlignment := 2;
          V.Cells[row,  7].HorizontalAlignment := 3;
          V.Cells[row, 7 ].Borders[10].ColorIndex := color_; // правая
          V.Cells[row, 8 ].Borders[10].ColorIndex := color_; // правая
          V.Cells[row, 9 ].Borders[10].ColorIndex := color_; // правая
...
//         oldpost :=cdsTemp.Fields[0].AsString;
     ...
     inc(row);
          ...
          MyRange.Borders.Weight := 2;
          MyRange.Borders.ColorIndex := 1;
          V.Cells[row, 1 ].Borders[10].ColorIndex := color_; // правая
          V.Cells[row, 5 ].Borders[10].ColorIndex := color_; // правая
          V.Cells[row, 6 ].Borders[10].ColorIndex := color_; // правая
          V.Cells[row, 7 ].Borders[10].ColorIndex := color_; // правая
          V.Cells[row, 8 ].Borders[10].ColorIndex := color_; // правая
...
{    inc(row);
    inc(row);
      V.Cells[row, 2 ].NumberFormat := StrFormat;
      V.Cells[row, 2 ].Value := 'Всего по филиалу';
...
{      inc(row);
      inc(row);
      V.Cells[row, 2 ].NumberFormat := StrFormat;
      V.Cells[row, 2 ].Value := 'Главный бухгалтер';
      inc(row);
      inc(row);
      V.Cells[row, 2 ].NumberFormat := StrFormat;
      V.Cells[row, 2 ].Value := 'Директор  Департамента по работе';
      inc(row);
      V.Cells[row, 2 ].NumberFormat := StrFormat;
      V.Cells[row, 2 ].Value := 'с персоналом';
}
      inc(row);
      inc(row);
    ...
    MyRange.Borders.ColorIndex := color_;
      V.Cells[row, 2 ].NumberFormat := StrFormat;
      V.Cells[row, 2 ].Value := 'Департамент Директори';
      inc(row);
...
// если все семь пустые значит его нигде нет!
  MemoProt.lines.add('');
             MemoProt.lines.add(str_);
{
...
//           MemoProt.lines.add('5)'+Sheet.Cells[j,5].value);
  ...
  MemoProt.lines.add('26700');
             line_:='Код должности='+trim(vartostr(Sheet.Cells[j,3].value))+', категория='+'1'+ ', код разряда='+razrad_code_+ 'невозможно ввести так как он уже есть в файле';
...
//           MemoProt.lines.add('6)'+Sheet.Cells[j,6].value);
             ...
             line_:='Код должности='+trim(vartostr(Sheet.Cells[j,3].value))+', категория='+'2'+ ', код разряда='+razrad_code_+ 'невозможно ввести так как он уже есть в файле';
...
//           MemoProt.lines.add('7)'+Sheet.Cells[j,7].value);
             ...
             line_:='Код должности='+trim(vartostr(Sheet.Cells[j,3].value))+', категория='+'3'+ ', код разряда='+razrad_code_+ 'невозможно ввести так как он уже есть в файле';
...
//           MemoProt.lines.add('8)'+Sheet.Cells[j,8].value);
             ...
             line_:='Код должности='+trim(vartostr(Sheet.Cells[j,3].value))+', категория='+'4'+ ', код разряда='+razrad_code_+ 'невозможно ввести так как он уже есть в файле';
...
//           MemoProt.lines.add('9)'+Sheet.Cells[j,9].value);
             ...
             line_:='Код должности='+trim(vartostr(Sheet.Cells[j,3].value))+', категория='+'5'+ ', код разряда='+razrad_code_+ 'невозможно ввести так как он уже есть в файле';
...
//           MemoProt.lines.add('10)'+Sheet.Cells[j,10].value);
             ...
             line_:='Код должности='+trim(vartostr(Sheet.Cells[j,3].value))+', категория='+'6'+ ', код разряда='+razrad_code_+ 'невозможно ввести так как он уже есть в файле';
...
//           MemoProt.lines.add('11)'+Sheet.Cells[j,11].value);
             ...
             line_:='Код должности='+trim(vartostr(Sheet.Cells[j,3].value))+', категория='+'7'+ ', код разряда='+razrad_code_+ 'невозможно ввести так как он уже есть в файле';
           ...
           do begin
             execsql('rollback');
             showmessage('ошибка)'+line_+#13+e.Message);
             exit;
           end;
         end;
...
begin
 ...
 Try
  ...
  try
    ...
    inc(row_j);
    V.Cells[row,row_j].Value:=' Категория ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' Коэффициент ';
    inc(row_j);
    V.Cells[row,row_j].ColumnWidth := 80; //width:=200;  пачиму не работает width ????
...
//
        ...
        while not ExecPrc.eof
        do begin
//    	МФО	Район	Категория	Коэффициент	BANK_NAME	REGION_NAM
...
begin
 ...
 Try
  ...
  try
    ...
    row_j:=1;
// уровень	Код	Должность	1 Категория	2 Категория	3 Категория	4 Категория	5 Категория	6 Категория	7 Категория

    V.Cells[row,row_j].Value:='  ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' Уровень ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' Код ';
    inc(row_j);
    ...
    inc(row_j);
    V.Cells[row,row_j].Value:=' 1 Категория ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' 2 Категория ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' 3 Категория ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' 4 Категория ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' 5 Категория ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' 6 Категория ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' 7 Категория ';
    inc(row_j);
...
////////////////////////
    ...
    then begin
      ...
      then begin
        mfo_category_type_code :='0';
        showmessage('(29290)Укажите категорию филиала '+branch+' в халк банке.(заполните Справочник Категорий Филиалов)');
      end else begin
        mfo_category_type_code :=vartostr(mfo_category_type_code_VAR) ;
      end;
    end else begin
...
////////////////////////
    ...
    // Source сделать лоаддата обязательно особенно если активный
          Source.close;
          with Source.Params
          do begin
              clear;
...
// без фильтра
 ...
 Try
  ...
  inc(row); MyRange:= V.Range[numto(1)+inttostr(row)+':'+numto(5)+inttostr(row)];  MyRange.Merge;
    ...
    V.Cells[row,row_j].ColumnWidth := 20;
    V.Cells[row,row_j].WrapText := true;
    V.Cells[row,row_j].Value:=' Кол-во шт.ед. ';
    inc(row_j);

    V.Cells[row,row_j].ColumnWidth := 20;
    V.Cells[row,row_j].WrapText := true;
    V.Cells[row,row_j].Value:=' Сумма окладов ';
    inc(row_j);

    V.Cells[row,row_j].ColumnWidth := 20;
    V.Cells[row,row_j].WrapText := true;
    V.Cells[row,row_j].Value:=' Сумма (худудий к.) ';
    inc(row_j);

    V.Cells[row,row_j].ColumnWidth := 20;
    V.Cells[row,row_j].WrapText := true;
    V.Cells[row,row_j].Value:=' Общая Сумма ';
    inc(row_j);
    row_j:=1;
    inc(row);
...
//     itogo_sum_:=0;
    ...
    then begin
      ...
      then begin
                inc(row);
                V.Cells[row,3].WrapText := true;
                V.Cells[row,3].Value := viloyat_name_+' Итого:';
      ...
      try
        ...
        if (headbranch_ ='09006')
        then begin
           // общая сумма окладов в халке
          // берем категорию филиала
            mfo_category_type_code_VAR := getsqlresultpar('select mfo_category_type_code from ss_ok_mfo_category'+
              ' where branch=:p_1',
            ...
            then begin
              ...
              showmessage('(27619)Укажите категорию филиала '+
                CDS_DBBranch.fieldbyname('branch').asstring+
                ' в халк банке.(заполните Справочник Категорий Филиалов)');
            end else begin
...
//-- и если там нет то сразу отдаем 0
          ...
          sum_koef_:=0;
           // общая сумма окладов не в халке
...
// без фильтра
 ...
 Try
  ...
  inc(row); MyRange:= V.Range[numto(1)+inttostr(row)+':'+numto(5)+inttostr(row)];  MyRange.Merge;
    ...
    V.Cells[row,row_j].ColumnWidth := 55;
    V.Cells[row,row_j].WrapText := true;
    V.Cells[row,row_j].Value:=' Наименование отдела ';
    inc(row_j);

    V.Cells[row,row_j].ColumnWidth := 55;
    V.Cells[row,row_j].WrapText := true;
    V.Cells[row,row_j].Value:=' Наименование должности ';
    inc(row_j);
    V.Cells[row,row_j].ColumnWidth := 55;
    V.Cells[row,row_j].WrapText := true;
    V.Cells[row,row_j].Value:=' Наименование специализации ';
    inc(row_j);
    V.Cells[row,row_j].ColumnWidth := 20;
    V.Cells[row,row_j].WrapText := true;
    V.Cells[row,row_j].Value:=' Кол-во шт.ед. ';
...
//     itogo_sum_:=0;
    ...
    then begin
      ...
      then begin
                inc(row);
                V.Cells[row,3].WrapText := true;
                V.Cells[row,3].Value := CDS_DBBranch.fieldbyname('REGION_NAM').asstring+' Итого:';
                V.Cells[row,4].Value := floattostr(itogo_count_viloyat_);
                V.Cells[row,5].Value := floattostr(itogo_sum_viloyat_);
                inc(row);
...
MemoProt :tmemo;
begin
// 22.05.2014 коэффициенты отделов - важен только код отдела
...
begin
 ...
 Try
  ...
  try
    ...
    V.Cells[row,row_j].Value:=' МФО ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' Код отдела ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' Наименование отдела ';
    inc(row_j);
    V.Cells[row,row_j].Value:=' Коэффициент ';
...
//  nn:=cdsTempSource.FieldCount-1;
  ...
  inc(i);
  V.Cells[row, i ].WrapText := True;
  V.Cells[row, i ].Value := 'Худудий коэффициент';
  V.Cells[row, i ].VerticalAlignment := 2;
  V.Cells[row, i ].HorizontalAlignment := 3;
  V.Cells[row, i ].Font.Bold := True;
  inc(i);
  V.Cells[row, i ].WrapText := True;
  V.Cells[row, i ].Value := 'Жами Ойлик иш хаки';
  V.Cells[row, i ].VerticalAlignment := 2;
  V.Cells[row, i ].HorizontalAlignment := 3;
  V.Cells[row, i ].Font.Bold := True;
