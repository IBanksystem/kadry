...
begin
  ...
  if ((HeadBranch_<>'09006') and (HeadBranch_<>'09004') and (HeadBranch_<>'09049')) // подтв. в ГО
  then begin
  ...
  if ((HeadBranch_ ='09006')  // подтв. в ГО
   or (HeadBranch_ ='09004') // 08.11.2012 теперь и в агро // подтв. в ГО
   or (HeadBranch_='09049'))  // подтв. в ГО
...
begin
  ...
  if  (gb_Obl.caption= ' (АРХИВ) Подтверждение в области (АРХИВ)')
...
//    ilova1.visible:=false;
  ...
  if (HeadBranch_ ='09006') or (HeadBranch_ ='09004')  // 08.11.2012 теперь и в агро // подтв. в ГО
  or (HeadBranch_='09049')  // подтв. в ГО
  then begin
...
//  sql_ := 'select * from okedb_appoints ';
  ...
  if (chbfiltr.checked) or (chbfiltrOBL.checked)
  then begin
    sql_1:='select b.gosubmit_name state_name ,a.* from okedb_appoints a, ss_ok_gosubmit b where a.state_code=b.gosubmit_code';
    sql_2:=' union all '+
           'select b.gosubmit_name state_name ,a.* from okedb_appoints_arh a, ss_ok_gosubmit b where a.state_code=b.gosubmit_code'
       ;
    gb_EDB_.caption:= ' (АРХИВ) Подтверждение в ГО (АРХИВ)';
    gb_Obl.caption:= ' (АРХИВ) Подтверждение в области (АРХИВ)';
    chbfiltr.checked   := true;
    ...
    sql_1 := 'select * from okedb_appoints where '+
