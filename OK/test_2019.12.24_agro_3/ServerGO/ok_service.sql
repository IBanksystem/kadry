...
IS
  Version constant char(12) := '#~24122019~#';
...
procedure OKCHANGEPASSPORT(
  ...
  emp_code_           OK_PERSONAL.EMP_CODE%type := param.getparam('EMP_CODE'),
  PASS_DATE_          OK_PERSONAL.PASS_DATE%type := param.getparam('PASS_DATE'),
  PASS_DATE_END_      OK_PERSONAL.PASS_DATE_END%type := param.getparam('PASS_DATE_END')
);
...
CREATE OR REPLACE PACKAGE BODY ok_service IS
  Version constant char(12) := '#~24122019~#';
--  24.07.2018 теперь  в агро без подтв. в ГО
--  30.06.2017 теперь  в Давр без подтв. в ГО
...
IS
  ...
  mes_ varchar2(2000);
  ZP_RATE_CODE_new_  OK_PERSONAL.ZP_RATE_CODE%type;
BEGIN
  ZP_RATE_CODE_new_ := param.getparam('P_STAVKA');
--
  if p_branch_ is null
  then branch_:=p_branch_;
  else branch_:=info.getbranch;
...
-- через количество должностей в OK_STAT
  select nvl(sum(quantity_post),0) into QuantityPost_
  from ok_stat
  where branch=branch_
  and department_code= department_code_new_
  and post_code = post_code_new_;
--
  if QuantityPost_ is null 
  then QuantityPost_:=0;
  end if;
--
  select nvl(((sum(nvl(zp_rate_code,8)))/8),0) into QuantityPostReal_
  from ok_personal
  where branch=branch_
  and department_code= department_code_new_
  and post_code = post_code_new_
  and status_code=2;
--
  if QuantityPostReal_ is null 
  then QuantityPostReal_:=0;
  end if;
--
  ...
  then
    ...
    for rec in (select *
      ...
      and status_code=2)
    loop
      mes_:=substr(mes_||'('||rec.tabno||'('||(nvl(rec.zp_rate_code,8)/8)||')'||')'||rec.family||'; ',1,1900);
    end loop;
    raise_application_error( -20000, 'В штатном расписании указано '||To_char(QuantityPost_)||
      ' рабочих мест. Фактически назначено '||to_char(QuantityPostReal_)||' едениц сотрудников:'||mes_||' Операция Назначение прервана!');
  end if;
--
  insert into okedb_appoints_arh
  select * from okedb_appoints
...
-- проставить на какую должность назначают
  update ok_personal set
    ...
    ZP_RATE_CODE_old = ZP_RATE_CODE,
    ZP_RATE_CODE_new = ZP_RATE_CODE_new_,
...
-- сохранить заявку в таблицу ГО
  insert into okedb_appoints (id,branch,PERSONAL_CODE,v_date,EMP_CODE,
    ins_date,state_code,comments,DEPARTMENT_CODE,POST_CODE,
    SPECIAL_CODE,DEPARTMENT_CODE_NEW,POST_CODE_NEW,SPECIAL_CODE_NEW,FAMILY,
    FIRST_NAME,PATRONYMIC,BIRTHDAY,tabno,ZP_RATE_CODE) select okedb_appoints_seq.nextval,
      branch, PERSONAL_CODE,v_date_,EMP_CODE,
    sysdate, is_goSended29 ,null,department_code,post_code,
    SPECIAL_CODE,DEPARTMENT_CODE_new,POST_CODE_new,SPECIAL_CODE_new,FAMILY,
    FIRST_NAME,PATRONYMIC,BIRTHDAY,tabno,ZP_RATE_CODE_new
    from ok_personal
    where branch=branch_ and personal_code=personal_code_;
--
END;
...
begin
  select sum(nvl(zp_rate_code,8))/8 into quantity_
  from ok_personal
...
IS
 GPH_DATE_BEGIN_      OK_PERSONAL.GPH_DATE_BEGIN  %type := param.getparam('GPH_DATE_BEGIN');
 GPH_DATE_END_      OK_PERSONAL.GPH_DATE_END  %type := param.getparam('GPH_DATE_END');
 GPH_SUMMA_      OK_PERSONAL.GPH_SUMMA  %type := param.getparam('GPH_SUMMA');
 GPH_POST_CODE_      OK_PERSONAL.GPH_POST_CODE  %type := param.getparam('GPH_POST_CODE');
 GPH_DEPARTMENT_CODE_      OK_PERSONAL.GPH_DEPARTMENT_CODE  %type := param.getparam('GPH_DEPARTMENT_CODE');
 GPH_ORDER_DATE_      OK_PERSONAL.GPH_ORDER_DATE  %type := param.getparam('GPH_ORDER_DATE');
 GPH_ORDER_NUM_      OK_PERSONAL.GPH_ORDER_NUM  %type := param.getparam('GPH_ORDER_NUM');
 GPH_REZIDENT_      OK_PERSONAL.GPH_REZIDENT  %type := param.getparam('GPH_REZIDENT');
...
--BOOK_SERIES,TELEFON,EMP_CODE,INS_DATE
 ...
 GPH_DATE_END ,
 GPH_SUMMA ,
 GPH_POST_CODE ,POST_CODE ,
 GPH_DEPARTMENT_CODE ,DEPARTMENT_CODE,
 GPH_ORDER_DATE ,
 GPH_ORDER_NUM ,
 GPH_REZIDENT
 ...
 GPH_DATE_END_ ,
 GPH_SUMMA_ ,
 GPH_POST_CODE_ ,GPH_POST_CODE_ ,
 GPH_DEPARTMENT_CODE_ ,GPH_DEPARTMENT_CODE_,
 GPH_ORDER_DATE_ ,
 GPH_ORDER_NUM_ ,
 GPH_REZIDENT_
      );
      ...
      if (head_='09053')
      then
        ESTABLISHED_POST_:= ok_service.get_postname(GPH_POST_CODE_);
        ESTABLISHED_DEPARTMENT_ := ok_service.get_FULLdepartmentname(GPH_DEPARTMENT_CODE_);
        DEPARTMENT_ := ok_service.get_departmentname(GPH_DEPARTMENT_CODE_);
        insert into ok_period ( BRANCH, PERSONAL_CODE,
          ...
          BRANCH_, in_personal_code_,    GPH_DATE_BEGIN_,    GPH_DATE_END_, ' ',
          ' ', ESTABLISHED_POST_, ESTABLISHED_DEPARTMENT_, -- наименование отдела
          is_this_bank_, null, GPH_ORDER_NUM_, GPH_ORDER_DATE_,
          EMP_CODE_, sysdate,2, GPH_POST_CODE_, GPH_DEPARTMENT_CODE_, BRANCH_,department_
        );
--
        send_ok_by_email(BRANCH_, in_PERSONAL_CODE_,4);
--
      end if;
    end if;
  else
...
--
  ...
  return boolean is begin
       select /*+ index ( c ok_personal_passport)*/ c.id into oldId_
       from ok_personal c where c.pass_num = pass_num_ and upper(c.pass_seriya) = upper( trim( pass_seriya_)) and  rownum<2;
...
--
  function isNewPassDate
  return boolean is begin
  -- Ипак йули меняет серию паспорта с кириллицы на латиницу 26.08.2016
  --     select /*+ index ( c ok_personal_passport)*/ c.id into oldId_
  --     from ok_personal c where c.personal_code=personal_code_
  --     and pass_date=pass_date_ and  rownum<2;
  --     raise_application_error( -20000, 'У Сотрудника уже указана'||
  --       ' дата выдачи паспорта '||to_char(pass_date_,'dd.mm.yyyy')||
  --      ' 0111347Код ошибки: 05406 (При изменении серии\номера паспорта, необходимо изменить дату выдачи паспорта) Необходимо комплексное изменение');
    return true;
  exception when no_data_found then return true;
  end isNewPassDate;
--
...
--
  if not isNewPassDate
  then
       raise_application_error( -20000, 'Сотрудник с паспортом серия:'|| upper(pass_seriya_) ||' номер:'|| pass_num_||
         ' и дата выдачи паспорта '||to_char(pass_date_,'dd.mm.yyyy')||' уже зарегистрирован в подсистеме ОК.'||
         ' 0111347Код ошибки: 05406 (При изменении серии\номера паспорта, необходимо изменить дату выдачи паспорта) Необходимо комплексное изменение');
  end if;
--
    update ok_personal
     set pass_seriya = pass_seriya_,
         pass_num = pass_num_,
         emp_code = emp_code_,
         PASS_DATE = PASS_DATE_,
         PASS_DATE_END = PASS_DATE_END_
   where branch= branch_
     and personal_code= personal_code_;
--
...
IS
user_name_ ok_personal.user_name%type;
...
-- проверка разрешения ГО 08.11.2012 .
  if (head_ ='09006') or (head_ ='09049') or (head_ ='09014') or (head_ ='*****') or (head_ ='*****') -- 22,04,2014 теперь в агро подтв. в ГО на увольнение НЕ Нужно.
...
--      and (head_ ='09006')  --((head_ ='09006') or (head_ ='09004')) --  агро подтв. в ГО На увольнение Нужно
      and ((head_ ='09006') or (head_ ='09049') or (head_ ='09014') or (head_ ='*****') or (head_ ='*****') )
--  and (branch_ in ('00000','00703'))
    then
...
--
  if (head_ ='09053') or (head_ ='09004') -- 04.07.2019 Инфин  блокировать уволенного пользователя
  then
    begin
      select  upper(USER_NAME) into user_name_ from ok_personal
           where branch = branch_
      and personal_code = personal_code_;
      update users set locked=1, pwd_expired=info.getday-10
...
--    alter user user_name_ account lock;
    if (user_name_ <>'*') and (user_name_ is not null)
    then
       EXECUTE IMMEDIATE 'alter user '||user_name_||' account lock';
    end if;
  end if;
  if head_= '09053' -- infin
  then
      send_ok_by_email(BRANCH_, PERSONAL_CODE_,3);
  end if;  
...
--
  ...
  into head_
  from s_mfo s
  where bank_id=branch_;
--
  if head_= '09053' -- infin
  then
    send_ok_by_email(BRANCH_, PERSONAL_CODE_,4);
  end if;
--
END;-- dekret
--
...
IS
  ...
  ZP_RATE_CODE_    OK_PERSONAL.ZP_RATE_CODE%type := param.getparam('STAVKA');
...
--
--  if k_=IS_NEW_STATUS_ then
--     raise_application_error( -20000, 'Сотрудник новый. Обратитесь в Республиканский Департамент Отдел Кадров. Операция Назначение прервана.');
--  end if;
--
  select department_code,       post_code,      gosubmit_code,
         PASS_SERIYA ,PASS_NUM ,PASS_DATE ,id_
    into department_code_old_,  post_code_old_, gosubmit_code_,
         PASS_SERIYA_,PASS_NUM_,PASS_DATE_,id_
    from ok_personal
       where branch = branch_
       and personal_code= personal_code_;
--  select count(1) into obl_ from s_mfo where bank_id=branch_ and region_id in ('03','10','22');
-- проверка разрешения ГО
  if   (head_ ='09006') or (head_ ='09004') -- and obl_>0 )
-- 30.04.2019 теперь в агро подтверждение в ГО  одному сотруднику по всему банке а не только только в трех областях
  or   (head_ ='09049') or (head_ ='09014')
  then
--
...
--
      if  (okedb_appoints_state_code_ <> gosubmit_code_)
      and ((head_ ='09006') or (head_ ='09004') -- and obl_>0) -- 17.04.2019 теперь и в агро подтверждение в ГО
      and  (head_ ='09049') or (head_ ='09014') or (head_ ='*****') or (head_ ='*****') )
--  and (branch_ in ('00000','00703'))
      then
...
-- поменял 29.11.2013 if (k_ = is_new_status_) and ((gosubmit_code_ <> is_gosubmitOK) or (gosubmit_code_ is null))
      ...
      and ((head_ ='09006') or (head_ ='09004') -- and obl_>0) -- 08.11.2012 теперь и в агро подтверждение в ГО
      and  (head_ ='09049') or (head_ ='09014') or (head_ ='*****') or (head_ ='*****') )
--  and (branch_ in ('00000','00703'))
      then
...
--
  ...
  then
    raise_application_error( -20000, 'В штатном расписании указано '||To_char(QuantityPost_)||
      ' рабочих мест. Фактически назначено '||to_char(QuantityPostReal_)||' едениц сотрудников. Операция Назначение прервана!');
  end if;
--
...
--
  if (status_code_ = IS_NEW_STATUS_)
  or (status_code_ = DISMISS_STATUS_CODE_)
  then
    delete from ok_hist where branch=BRANCH_ and personal_code=PERSONAL_CODE_;
...
--
  ...
  then
    if (status_code_ = is_new_status_)
    then
      send_ok_by_email(BRANCH_, PERSONAL_CODE_,1);
    else
      send_ok_by_email(BRANCH_, PERSONAL_CODE_,2);
    end if;
  end if;  
--
