CREATE OR REPLACE PACKAGE ok_service AUTHID CURRENT_USER IS
  Version constant char(12) := '#~16062011~#';
...
--
  ...
  then
    select post_group_code into post_group_code_
    from ss_ok_post where post_code=post_code_;
    if (post_group_code_=1) and (post_group_code_ is not null)
    then
