CREATE OR REPLACE TRIGGER TR_OK_PERSONAL_HIST
 ...
 FOR EACH ROW
declare
Version constant char(12) := '#~24012018~#';
new_id           NUMBER;
...
-- 09_001
  ...
  if updating then
    ...
    OLD_department_code_ :=:NEW.department_code;
    id_table_   :=:OLD.ID;
    PASS_SERIYA_NEW_ :=:NEW.PASS_SERIYA;
    PASS_NUM_NEW_    :=:NEW.PASS_NUM;
    PASS_DATE_NEW_   :=:NEW.PASS_DATE;
  ...
  if deleting then
    ...
    PASS_DATE_OLD_   :=:OLD.PASS_DATE;
    old_post_code_   :=:OLD.POST_CODE;
    OLD_department_code_ :=:OLD.department_code;
    id_table_   :=:OLD.ID;
    PASS_SERIYA_NEW_ :=null;
    PASS_NUM_NEW_    :=null;
    PASS_DATE_NEW_   :=null;
    new_post_code_   :=null;
    NEW_department_code_ := null;
    id_table_   :=:OLD.ID;
  end if;
