...
begin
  ...
  loop
    ddIN := to_number( to_char(rec.in_office_date,'dd') );
    mmIN := to_number( to_char(rec.in_office_date,'mm') );
    yyyyIN := to_number( to_char(rec.in_office_date,'yyyy') );
--
    if (rec.OUT_OFFICE_DATE is null)
    or (rec.OUT_OFFICE_DATE>date_open_)
    then
      ddOUT := to_number( to_char( date_open_,'dd') );
      mmOUT := to_number( to_char( date_open_,'mm') );
      yyyyOUT := to_number( to_char( date_open_,'yyyy') );
      dd := dd + ddOUT - ddIN;
      if dd<0 then
        dd := dd + 30;
        mm := mm - 1;
...
--
      ...
      ddOUT := to_number( to_char(rec.out_office_date,'dd') );
      mmOUT := to_number( to_char(rec.out_office_date,'mm') );
      yyyyOUT := to_number( to_char(rec.out_office_date,'yyyy') );
      dd := dd + ddOUT - ddIN;
      if dd<0 then
        dd := dd + 30;
        mm := mm - 1;
