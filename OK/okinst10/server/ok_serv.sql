...
-- отрезок в начале стажа
    if to_char(min_IN_OFFICE_DATE_,'dd')='01'
    then -- первое число
      ddIN_count:=0;
      mm := mm+1;
    else
...
-- отрезок в конце стажа
    if last_day(date_open_-1)=(date_open_-1)
    then -- последний день месяца
      ddOUT_count:=0;
      mm := mm+1;
    else
      ddIN_count := (date_open_-1)- to_date('01.'||to_char(date_open_-1,'mm.yyyy') ,'dd.mm.yyyy')+1;
    end if;
...
-- месяцы
      mm:=1+mm+trunc(MONTHS_BETWEEN( to_date('01.'||to_char(date_open_-1,'mm.yyyy') ,'dd.mm.yyyy'),last_day(min_IN_OFFICE_DATE_)+1 ));
      ...
      -- отрезок в конце rec.in_office_date
        ...
        then -- последний день месяца
          ...
          mm_delta := mm_delta+1;
           ...
           ddIN_count := rec.in_office_date- to_date('01.'||to_char(rec.in_office_date,'mm.yyyy') ,'dd.mm.yyyy')+1;
         end if;
        dd_delta := ddIN_count+ddOUT_count;
      end if;
-- месяцы
      mm_delta:=1+mm_delta+trunc(MONTHS_BETWEEN(  to_date('01.'||to_char(rec.in_office_date,'mm.yyyy') ,'dd.mm.yyyy'),last_day(PREV_OUT_OFFICE_DATE_)+1 ));
      dd:=dd-dd_delta;
      mm:=mm-mm_delta;
      if dd<0 
      then
