...
--  cccc number;
begin
-- переброска данных с филиалов в IY09014 по отделу кадров в 12 часов ночи
-- удаление
      dest_table_name:='OK_ACADEMIC';
    execute immediate 'delete from IY09014.'||dest_table_name;
    commit;
...
-- вставка  
    for rec in (Select User_Name,
             Db_Alias, branch
       from  IY09014.SS_DBLink_Branch WHERE BRANCH<>'09004')
    loop
      dest_table_name:='OK_ACADEMIC';
      sour_table_name:=rec.User_Name||'.'||dest_table_name;
      execute immediate 'insert into IY09014.'||dest_table_name||' select * from '||sour_table_name||' where branch='''||rec.branch||'''';
      commit;
      dest_table_name:='OK_ADDINFORM';
      sour_table_name:=rec.User_Name||'.'||dest_table_name;
      execute immediate 'insert into IY09014.'||dest_table_name||' select * from '||sour_table_name||' where branch='''||rec.branch||'''';
