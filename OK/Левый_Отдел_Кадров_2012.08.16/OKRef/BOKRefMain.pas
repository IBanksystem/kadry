...
//
  ...
  SQL_SSOKcategory_SET       = 'select CATEGORY_RATE,SALARY,STATE_NOTES,EMP_CODE,INS_DATE,category_code,to_char(category_name) category_name from SS_OK_category where state_notes=''A''';
...
var
  ...
  FiltrLevelName, FiltrLevel, BossLevel   : String  ;
...
// обл
   ...
   do begin
    ...
    LBlevel.caption := FiltrLevelName+' - доступ Только для чтения';
  end else begin
...
//  PC.ActivePage := TabMessage;
  OldStateNotes := 'A';
  if ( (HeadBranch='09004') and (branch<>'00394') )
  or ( (HeadBranch='09006') and (branch<>'00883') and (branch<>'09006') )
  then begin
    showmessage('Справочники Ведутся Только В Республиканском Отделе Кадров.');
    exit;
  end;
...
begin
  ...
  PC.ActivePage := TabMessage;
  if ( (HeadBranch='09004') and (branch<>'00394') )
  or ( (HeadBranch='09006') and (branch<>'00883') )
  then begin
    showmessage('Справочники Ведутся Только В Республиканском Отделе Кадров.');
    exit;
  end;
...
// showmessage( inttostr(Integer(TreeV.Selected.data)) );
  if Integer(TreeV.Selected.data)=0
  then begin
    showmessage('Нельзя Удалить (Деактивировать) отдел '+TreeV.Selected.text);
    exit;
  end;

  If Application.MessageBox(PChar('ВЫ УВЕРЕНЫ,'+#13+
    ' что хотите Удалить (Деактивировать) отдел '+TreeV.Selected.text+' ?'+#13+
  ...
  Then Begin
   ...
   TRY
    ...
    sql_ := 'update ss_ok_department'+
            ' set state_notes=''P'' '+
            ' where branch='''+Branch+''' '+
            ' and department_code=' +  cdsDerevo.FieldByname('department_code').AsString  ;
//showmessage(sql_);
    try
      ExecSQL(sql_);
      ExecSQL('COMMIT');
   ...
   END;
    showmessage('Отдел Удален (деактивирован).');
...
//cdsSSOKcategory_Set.FieldByName('salary').asstring
  If Application.MessageBox(PChar('Вы уверены?,'+#13+
    ...
    ' '), PChar('ПРЕДУПРЕЖДЕНИЕ!'), MB_YESNO or MB_ICONQUESTION) = mrYes
  Then Begin
    if ( (HeadBranch='09049') or (HeadBranch='00974') )
    then begin
   // капитал банк
...
var
  ...
  s, rez, col,sell_address:String;
  llin, ccol, cc,ll:integer;
  tabno_,gender : string;
...
var sql1_, sql2_, sql3_, sql_: string;
//  salary_0:Double;
begin
//  salary_0:=getsqlresult('select salary from ss_ok_min_zp '+
//      ' where date_open in (select max(date_open) from ss_ok_min_zp)');
//cdsSSOKcategory_Set.FieldByName('salary').asstring
//  If Application.MessageBox(PChar('Вы уверены?'), PChar('ПРЕДУПРЕЖДЕНИЕ!'), MB_YESNO or MB_ICONQUESTION) = mrYes
//  Then Begin
...
{  if ( (HeadBranch='09049') or (HeadBranch='00974') )
   ...
   // капитал банк
     sql1_:='update ss_ok_min_zp set '+
      ' category_zero_salary=round(salary*rate),EMP_CODE='+userid+','+
      ' INS_DATE=sysdate'+
      ' where date_open in (select max(date_open) from ss_ok_min_zp)';
   end else begin
...
var s,sql_ ,oneline, table_name,field_list,FileName,isermes,from_niki: string;
 i2,k,i : integer;
 f: TextFile;
begin
...
var s,sql_ ,oneline, table_name,field_list,FileName,isermes: string;
 i2,k : integer;
 f: TextFile;
begin
