...
-- дата 
-- селект
prompt # код сотрудников через запятую(74,98,170) 4 раза, дата один раз 
...
select okp.* from ok_personal okp, SS_OK_NCI son 
  where okp.tabno=okp.tabno
   ...
   and okp.personal_code in (74,98,170);
--
declare
...
begin
  ...
  delete from ok_hist where branch=branch_ and personal_code in (74,98,170);
...
/
  ...
  commit;
select * from ok_hist where branch in (select bank_id  from branch) and personal_code  in (74,98,170);
