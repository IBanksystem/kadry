...
-- дата 
-- селект
prompt # код сотрудника personal_code сменить четыре раза, дата один раз 
...
begin
  ...
  delete from ok_hist where branch=branch_ and personal_code=68;
...
/
  ...
  commit;
select * from ok_hist where branch in (select bank_id  from branch) and personal_code= 68;
