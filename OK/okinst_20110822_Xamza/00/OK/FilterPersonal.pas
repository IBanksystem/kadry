  ...
  Case Key of
    VK_RETURN: SelectNext(ActiveControl, True, True);
        VK_F9: Begin
                 ...
                 Then BOKForm.LoadData(Source,'SELECT to_char(department_code) id, department_name name'+
                                              '  FROM ss_ok_department'+
                                              ' WHERE state_notes = ''A''',True);
                 If Sender = edPost
                 Then BOKForm.LoadData(Source,'SELECT to_char(post_code) id, post_name name'+
                                              '  FROM ss_ok_post'+
                                              ' WHERE state_notes = ''A''',True);
                 If Sender = edSpecial
                 ...
                 Then BOKForm.LoadData(Source,'SELECT to_char(education_code) id, education_name name'+
