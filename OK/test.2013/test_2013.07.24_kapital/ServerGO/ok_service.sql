CREATE OR REPLACE PACKAGE ok_service AUTHID CURRENT_USER IS
  Version constant char(12) := '#~24072013~#';
...
CREATE OR REPLACE PACKAGE BODY ok_service IS
  Version constant char(12) := '#~24072013~#';
--  08.11.2012 теперь и в агро подтверждение в ГО
...
-- сохранить заявку в таблицу ГО
  insert into okedb_appoints (id,branch,PERSONAL_CODE,v_date,EMP_CODE,
    ...
    ins_date, is_forDismiss30 ,null,DEPARTMENT_CODE,POST_CODE,
    SPECIAL_CODE,DEPARTMENT_CODE_new,POST_CODE_new,SPECIAL_CODE_new,FAMILY,
...
-- сохранить заявку в таблицу ГО
  insert into okedb_appoints (id,branch,PERSONAL_CODE,v_date,EMP_CODE,
    ...
    FIRST_NAME,PATRONYMIC,BIRTHDAY,tabno) select okedb_appoints_seq.nextval,
      branch, PERSONAL_CODE,v_date_,EMP_CODE,
    ins_date, is_goSended29 ,null,department_code,post_code,
    SPECIAL_CODE,DEPARTMENT_CODE_new,POST_CODE_new,SPECIAL_CODE_new,FAMILY,
...
-- проверка разрешения ГО 08.11.2012 .
  if (head_ ='09006') --or (head_ ='09004') в агро подтверждение в ГО на увольнение не нужно.
...
--      and (head_ ='09006')  --((head_ ='09006') or (head_ ='09004')) -- 08.11.2012 теперь и в агро подтверждение в ГО
      and ((head_ ='09006') or (head_ ='09049'))
--  and (branch_ in ('00000','00703'))
    then
...
--
      ...
      and ((gosubmit_code_ <> is_gosubmitOK) or (gosubmit_code_ is null))
--      and (head_ ='09006') --or (head_ ='09004')) -- 08.11.2012 теперь и в агро подтверждение в ГО
      and ((head_ ='09006') or (head_ ='09049'))
--  and (branch_ in ('00000','00703'))
      then
...
-- вставка в ok_period
  insert into ok_period ( BRANCH, PERSONAL_CODE,
    ...
    INS_DATE, base_move_code, post_code, department_code, Cod_Bank
  ) values (
    ...
    EMP_CODE_, sysdate,base_move_code_, post_code_, department_code_, BRANCH_
  );
--
