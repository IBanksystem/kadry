...
procedure TfmOKUpDep.bClearClick(Sender: TObject);
var i: Integer;
begin
...
procedure TfmOKUpDep.FormCreate(Sender: TObject);
begin
  BOKForm := TBOKForm(owner);

end;

procedure TfmOKUpDep.bOkClick(Sender: TObject);
var
  ErMessage : string;
const
  ERROR_NO = 'ORA-20000: ';
...
//        then begin
//          ErMessage := ErMessage +'Отчество'+#13;
//        end;

        if medChangeDate.Text=''
        then begin
          ErMessage := ErMessage +'Дата изменения ФИО'+#13;
        end;

        try
          StrToDate(medChangeDate.Text);
        except
          ErMessage := ErMessage +' Дата изменения ФИО "'+medChangeDate.Text+'" введена некорректно!'+#13;
        end;

        if medChangeMotive.Text=''
        then begin
          ErMessage := ErMessage +'Причина изменения ФИО'+#13;
        end;
...
//          execSQL('begin Param.SaveParam(1); end;');
          ...
          BOKForm.execSQL('begin Param.SetParam(''FAMILY'',    '''+ medFamily.Text + '''); end;');
          BOKForm.execSQL('begin Param.SetParam(''FIRST_NAME'','''+ medName.Text   + '''); end;');
          BOKForm.execSQL('begin Param.SetParam(''PATRONYMIC'','''+ medPatronymic.Text      + '''); end;');
