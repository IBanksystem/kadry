...
//    memoProt.lines.add(sql_);
    ExecSQL('begin info.init; end;');
//** ага и нет доступа потом пишет    ExecSQLpar('begin info.initbranch(:p_1); end;',old_branch);
//** потому что при входк обратно не было инфо-инита
//**    ExecSQL('alter session set NLS_DATE_FORMAT ='+''''+'dd.mm.yyyy'+'''');
    execsql('COMMIT');
    branch := old_branch;
...
begin
  ...
  if ((HeadBranch_ ='09006') or (HeadBranch_ ='09004')) // 08.11.2012 теперь и в агро подтверждение в ГО
...
begin
  ...
  if ((HeadBranch_ ='09006') or (HeadBranch_ ='09004'))  // 08.11.2012 теперь и в агро подтверждение ГО
  or (HeadBranch_='09049')
  then begin
...
begin
  ...
  then begin
    ...
    //  nn:=cdsTempSource.FieldCount-1;
      i:=1;
      // формирование шапки
      V.Cells[row, i ].Value := 'Назорат';
      inc(i);
      V.Cells[row, i ].Value := 'Лавозим';
      inc(i);
      V.Cells[row, i ].Value := 'Штат';
      inc(i);
      V.Cells[row, i ].Value := 'Лавозим';
      inc(i);
      V.Cells[row, i ].Value := 'Сувсизлик';
      inc(i);
      V.Cells[row, i ].Value := 'Иш хакининг';

      inc(row);
      i:=1;
      V.Cells[row, i ].Value := 'т.р.';
      inc(i);
      V.Cells[row, i ].Value := 'номи';
      ...
      inc(i);
      V.Cells[row, i ].Value := 'ойлик фонди';
...
//
        ...
        do begin
          ...
          ExecPrc.next;
          // в следующей строке колво по факту
