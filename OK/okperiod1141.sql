spool c:\BANK1141.txt;

alter table BANK1141.OK_PERIOD add MOTIVE_OUT             VARCHAR2(150);

drop trigger BANK1141.TR_OK_PERIOD_HIST;

alter table BANK1141.OK_PERIOD
  drop constraint OK_PERIOD_PK;
drop index  OK_PERIOD_PK;

alter table BANK1141.OK_PERIOD
  drop constraint OK_PERIOD_UK;
drop index  BANK1141.OK_PERIOD_UK;




drop table ok_period_old;
alter table BANK1141.ok_period rename to ok_period_old;
create table ok_period as select * from BANK1041.ok_period where rownum<1;
commit;
insert into BANK1141.ok_period (
 ID, 
 BRANCH, 
 PERSONAL_CODE,
 IN_OFFICE_DATE, 
...
alter table OK_PERIOD add constraint OK_PERIOD_PK primary key (BRANCH, PERSONAL_CODE, IN_OFFICE_DATE);

alter table OK_PERIOD add constraint OK_PERIOD_UK unique (BRANCH, PERSONAL_CODE, OUT_OFFICE_DATE);
