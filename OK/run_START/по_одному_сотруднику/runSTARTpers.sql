...
-- дата 
declare
  V_DATE_ date :=to_date('12.06.2012','dd.mm.yyyy');
  branch_ branch.bank_id%type;
  pers_ ok_personal.personal_code%type := 12345;  -- код сотрудника без мфо
begin
  select bank_id into branch_ from branch;
  delete from ok_hist where branch=branch_ and personal_code= pers_;
