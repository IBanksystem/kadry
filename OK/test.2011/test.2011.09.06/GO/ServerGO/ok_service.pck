CREATE OR REPLACE PACKAGE ok_service AUTHID CURRENT_USER IS
  Version constant char(12) := '#~19082011~#';
...
-- сотрудник отправлен в ГО
  is_go_to_GO constant ok_personal.gosubmit_code%type := 29;
-- сотрудник Подтвержден в ГО
  is_get_in_GO constant ok_personal.gosubmit_code%type := 22;
-- сотрудник Утвержден ГО
  is_gosubmit constant ok_personal.gosubmit_code%type := 33;
-- сотрудник отклонен ГО
  is_gorefused constant ok_personal.gosubmit_code%type := 14;
-- сотрудник удален ГО
...
--
  NCI_ID_POST  constant number(2):=79; -- код справочника должностей 
--
-- Person      Date         Comments
-- ---------   ----------   ----------------------------------------
-- ildar-y     29-04-2004   Отдел Кадров.Процедуры
...
begin
  ...
  AND ((department_code= department_code_
      ...
      and status_code=IS_JOIN_STATUS_) -- 2
    OR ((department_code= department_code_
      and post_code = post_code_
      and (special_code = special_code_
          or (special_code is null and special_code_ is null))
      and status_code in (is_go_to_GO,is_get_in_GO) ))   -- 29,22
      )
  ;
  return quantity_;
  EXCEPTION when NO_DATA_FOUND then
...
--        IN_OFFICE_DATE_OLD  ok_post.appointment_date%type;
        POST_CODE_  ss_ok_post.post_code%type;
...
-- проверка разрешения ГО
  select gosubmit_code,post_code into gosubmit_code_,post_code_
  from ok_personal
     where branch = branch_
     and personal_code= personal_code_;
--
  if  ((gosubmit_code_ <> is_gosubmit_del6) or (gosubmit_code_ is null))
    and (head_ ='09006')
  then
    select count(*) into post_count_
    from ss_ok_nci where NCI_ID=79 and OK_ID=post_code_;
    if post_count_>0
    then
...
--  salary_code_  ok_personal.salary_code%type;
 ...
 head_ varchar2(5);
 post_group_code_  ss_ok_post.post_group_code%type ;
...
--
-- проверка разрешения ГО
  select gosubmit_code, post_code into gosubmit_code_,old_post_code_
  from ok_personal
     where branch = branch_
     and personal_code= personal_code_;
--
  if (head_ ='09006') --(k_ = is_new_status_)
  then
    select count(*) into post_group_code_
    from ss_ok_nci where NCI_ID=NCI_ID_POST --79 
    and (OK_ID=post_code_ or OK_ID=old_post_code_);
    if (post_group_code_>0) and (gosubmit_code_ <> is_gorefused )
    and (gosubmit_code_ <> is_gosubmit) --or (gosubmit_code_ is null) если пусто то можно
    and (gosubmit_code_ is not null)
    then
...
--
-- запомнить предидущее место работы
  update ok_personal
  set department_code_old= department_code,
      post_code_old= post_code,
      special_code_old = special_code
   where branch = branch_
     and personal_code= personal_code_;
...
-- 
  if (head_ ='09006') and (post_group_code_>0) --(k_ = is_new_status_)
  then
    update ok_personal
    set department_code= department_code_,
      post_code= post_code_,
      special_code = special_code_,
      status_code= is_go_to_GO,
      emp_code= emp_code_,
      gosubmit_code=is_go_to_GO -- отправлен в ГО
     where branch = branch_
     and personal_code= personal_code_;
  else
    update ok_personal
