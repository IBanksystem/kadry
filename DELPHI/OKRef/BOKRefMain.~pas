...
type
  TBOKRefForm = class(TBaseForm)
    ...
    Label24: TLabel;
    TabSSOKcategoryARH: TTabSheet;
    GroupBox48: TGroupBox;
    ClientDBGrid1: TClientDBGrid;
    CDSss_ok_category_arh: TClientDataSetEx;
    DSss_ok_category_arh: TDataSource;
    N7: TMenuItem;
    ...
    procedure cdsDetailFIELDGetText(Sender: TField; var Text: String;
      DisplayText: Boolean);
    procedure cdsSSOKcategory_SETBeforeInsert(DataSet: TDataSet);
...
//  SQL_SSOKcategory_SET       = 'select CATEGORY_RATE,SALARY,STATE_NOTES,EMP_CODE,INS_DATE,category_code,category_name from SS_OK_category where state_notes=''A'' order by category_code';
  SQL_SSOKcategory_SET       = 'select category_code, ''  ''||category_name category_name from SS_OK_category where state_notes=''A'' order by category_code';
  SQL_SS_OK_category_ARH       = 'select * from SS_OK_category order by category_code,v_date_from';
...
//  showMessage(convert('Сейчас я в головном банке09049')+#13#10+' '+'09049'+#13#10+#13#10+
  //   convert('Сейчас я в районном банке00974')+#13#10+' '+'00974');

//  showMessage(convert('Сейчас я в городском банке00394')+#13#10+' '+'00394');
//showMessage(convert('Сейчас я в городском банке01158')+#13#10+' '+'01158');

//  showMessage(convert('Сейчас я в головном банке01125')+#13#10+' '+'01125');
...
//  showMessage('Сейчас я в головном банке'+Branch +#13#10+
//      convert('Сейчас я в головном банке'+Branch)+#13#10+
//      Head_);
  if (Head_ <> convert('Сейчас я в головном банке' +Branch))
  and (branch<>'01142')
  then
  begin
...
//  StatusBar.Panels[0].Text :='Всего: '+IntToStr(j);
//  StatusBar.Panels[0].Text :='Всего: '+IntToStr(j)+' отделов';
  if (PC.ActivePage=TabSSOKnci)
  and (TDataS(CB_S.Items.Objects[CB_S.ItemIndex]).Id='79')
  then begin
    if (FiltrLevel='1')
    then begin
      cdsSSOKnci.filter:='s_id<100';
      cdsSSOKnci.filtered:=true;
    end;
    if (FiltrLevel='2')
    then begin
      cdsSSOKnci.filter:='s_id>100 and s_id<200';
      cdsSSOKnci.filtered:=true;
    end;
    if (FiltrLevel='3')
    then begin
      cdsSSOKnci.filter:='s_id>200';
      cdsSSOKnci.filtered:=true;
    end;
    if (FiltrLevel='4') or (FiltrLevel='5')
    then begin
      cdsSSOKnci.filtered:=false;
    end;
  end;
...
//showmessage(DBLCB_OK.ListField);
  try
    ...
    then begin
      sql_:='select post_code,post_name||'' (''||decode(level_department_code,1,''Р-си'',2,''В-ти'',3,''Т-ни'',5,''Ш-ри'',''---'')||'')'' post_name from '+TDataS(CB_S.Items.Objects[CB_S.ItemIndex]).SS_OK_TBL_NAME+
  ...
  try
    ...
    if TDataS(CB_S.Items.Objects[CB_S.ItemIndex]).Id='79'
    then begin
      sql_:= 'select dolj_id,dolj_name||'' (''||decode(urov_id,0,''Р-си'',1,''В-ти'',2,''Т-ни'',''---'')||'')'' dolj_name,urov_id from '+TDataS(CB_S.Items.Objects[CB_S.ItemIndex]).NCI_TBL_NAME+
...
//cdsSSOKcategory_Set.FieldByName('salary').asstring
  ...
  Then Begin
   ...
   // капитал банк
      ...
      ' INS_DATE=sysdate'+
      ' where category_name<>''0'' and state_notes=''A'' ';
      ExecSql(sql_);
      sql_:='update ss_ok_category set '+
...
var SQLSQL_, sql1_, sql2_, sql3_, sql_, sql0_: string;
begin
    ...
    sql0_:='insert into ss_ok_category select CATEGORY_CODE,  CATEGORY_NAME,  CATEGORY_RATE ,  SALARY,  STATE_NOTES,'+
          ' EMP_CODE,  INS_DATE,  info.getday,  USER_NAME from ss_ok_category where state_notes=''A'' '+
          ' and CATEGORY_CODE not in (select CATEGORY_CODE from ss_ok_category where v_date_from=info.getday)';

    sql_:='update ss_ok_category a set a.state_notes=''P'''+
          ' where a.v_date_from < (select max(b.v_date_from) from ss_ok_category b where a.category_code=b.category_code)'+
          ' and a.state_notes=''A'' ';
...
begin
  j:=0;
  try
    j:=GetSqlResult('select count(*) from ss_ok_privilege ');
  except
  end;
  StatusBar.Panels[0].Text :='Всего: '+IntToStr(j);
...
procedure TBOKRefForm.cdsDetailFIELDGetText(Sender: TField; var Text: String;
  DisplayText: Boolean);
begin
    Text := sender.AsString;
end;

procedure TBOKRefForm.cdsAfterOpen(DataSet: TDataSet);
var
  i: Integer;
begin
//  DataSet.FieldByName('V_DATE_FROM').EditMask  := '!09/09/0099;1;_';
//  DataSet.FieldByName('V_DATE_TO').EditMask  := '!09/09/0099;1;_';

  with DataSet do
  begin
    for i := 0 to Fields.Count - 1 do
      if (Fields[i].DataType = ftDateTime)
         or (Fields[i].DataType = ftDate)
      then
        Fields[i].EditMask := '!09/09/0099;1;_'
      else if (Fields[i].DataType=ftMemo) then
        Fields[i].OnGetText := cdsDetailFIELDGetText
      else if (Fields[i].FieldName = 'SUMMA')
                or (Fields[i].FieldName = 'SUMMA_RAZN')
                or (Fields[i].FieldName = 'OST')
                or (Fields[i].FieldName = 'TOTAL_SUMMA')
      then
...
procedure TBOKRefForm.N7Click(Sender: TObject);
var j : integer;
begin
  j:=0;
  try
    j:=GetSqlResult('select count(*) from ss_ok_category_arh ');
  except
  end;
  StatusBar.Panels[0].Text :='Всего: '+IntToStr(j);

  PC.ActivePage := TabSSOKcategoryARH;
  If cdsSS_OK_category_arh.Active Then Exit;
  cdsSS_OK_category_arh.DisableControls;
  LOADDATA(cdsSS_OK_category_arh,      SQL_SS_OK_category_ARH);
  cdsSS_OK_category_arh.EnableControls;

end;

procedure TBOKRefForm.cdsSSOKcategory_SETBeforeInsert(DataSet: TDataSet);
begin
//  DataSet.Fields.FieldByName('v_date_from').Asstring:=bankdate;
end;
