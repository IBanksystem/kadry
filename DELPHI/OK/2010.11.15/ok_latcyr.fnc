...
--         
         ...
         if (y>=65) and (y<=90)  then lat:=lat+1;   end if;
         if (y>=91) and (y<=96)  then znak:=znak+1; end if;
         if (y>=97) and (y<=122) then lat:=lat+1;   end if;
         --
    ...
    then return ' спец='||bad||' лат='||lat||' кир='||cyr||' текст='||v_str;
--    raise_application_error(-20000,' спец='||bad||' лат='||lat||' кир='||cyr||' текст='||v_str);
    else return '1';
    end if;
