...
type
  TfmOKAppoint = class(TForm)
    ...
    procedure edStavkaChange(Sender: TObject);
    procedure CBStavkaChange(Sender: TObject);
  private
                    ...
                    do begin
                        ...
                        then begin
                          ...
                          then begin
                            ...
                            showmessage('(213)Укажите категорию филиала в халк банке.(заполните Справочник Категорий Филиалов)');
                          end else begin
...
////////////
  ...
  BOKForm.LoadData(BOKForm.Source,'SELECT zp_rate_code id,'+
                     ' zp_rate_code||'') ''||zp_rate_name name'+
                     ' FROM ss_ok_zp_rate'+
                     ' WHERE state_notes=:p_1'+
                     ' order by zp_rate_code',True);
  CBStavka.Clear;
  ...
  begin
      ...
      CBStavka.Items.AddObject(BOKForm.Source.FieldByName('name').AsString, D);
      BOKForm.Source.Next;
  end;
...
//  then LoadData(cdsOKVacancy,SQL_Vacancy);
    ...
    BOKForm.LoadData(BOKForm.Source,'select a.department_code id,'+
      ' lpad(to_char(a.department_code),5,'' '')||'') ''||a.department_name name '+
  ...
  begin
      ...
      BOKForm.Source.Next;
            ...
            then begin
              ...
              then begin
                mfo_category_type_code :='';
                showmessage('(405)Укажите категорию филиала в халк банке.(заполните Справочник Категорий Филиалов)');
              end else begin
                mfo_category_type_code :=vartostr(mfo_category_type_code_VAR) ;
              end;
              CreateParam(ftString,'p_1' ,ptInput);
...
begin
  ...
  then begin
    ...
    BOKForm.LoadData(BOKForm.Source,'select a.post_code id, '+
          ...
          ' from ss_ok_post a where'+
          ' a.state_notes=:p_1'+ //''A'
// показать что есть в штатном а не по уровню         ' and a.level_department_code in (:p_2,:p_3)'+ //('+BossLevel+',4)'+
...
begin
  ...
  if  (headbranch_='09053')  // инфин
  then begin

  end else begin
        if BOKForm.cdsOKPersonal.fieldbyname('Education_Title_CODE').asstring=''
        then ErMessage:=ErMessage+' отсутствует * Образование Сотрудника ( Наименование ) на вкладке "Информация по сотруднику" ';
...
/////////////////
//  проверять что нет принятых сотрудников с таким ИНПС в других филиалах банка
    showmess_:='-';
    CheckPoisk;
    if showmess_<>'-'
...
//    BOKForm.execSQLpar('begin Param.SetParam(''EMP_CODE'','+ UserId +'); end;');
    ...
    BOKForm.execSQLpar('begin Param.SetParam(:p_1,:p_2); end;', 'STAVKA', edStavka.Text );
...
begin
  ...
  then begin
    ...
    showmessage('Проверка на наличие неуволенного сотрудника с таким же НПС в других филиалах - успешно. Ошибок нет.');  
  end;
end;
  ...
  BOKForm.Source2.First;
     ...
     do begin
                    ...
                    BOKForm.LoadData(BOKForm.Source,'SELECT branch,nps_id,family,first_name,patronymic,'+
                     ...
                     do begin
                         showmess_:=showmess_+'Найден неуволенный сотрудник '+#13+
                          'МФО:       '+  BOKForm.Source.FieldByName('branch').AsString+#13+
                          'НПС:       '+  BOKForm.Source.FieldByName('NPS_ID').AsString+#13+
                          'Фамилия:   '+  BOKForm.Source.FieldByName('family').AsString+#13+
                          'Имя:       '+  BOKForm.Source.FieldByName('first_name').AsString+#13+
                          'Отчество:  '+  BOKForm.Source.FieldByName('patronymic').AsString+#13+
                          'Отдел:     '+  BOKForm.Source.FieldByName('department_show').AsString+#13+
                          'Должность: '+  BOKForm.Source.FieldByName('post_show').AsString+#13+
                          'Специализация: '+  BOKForm.Source.FieldByName('special_show').AsString+#13+
                          'День рождения: '+  BOKForm.Source.FieldByName('birthday').AsString+#13
                          ;
                         memopoisk.lines.add(
                            'МФО:       '+  BOKForm.Source.FieldByName('branch').AsString
                         );
                         memopoisk.lines.add(
                            'НПС:       '+  BOKForm.Source.FieldByName('NPS_ID').AsString
                         );
                         memopoisk.lines.add(
                            'Фамилия:   '+  BOKForm.Source.FieldByName('family').AsString
                         );
                         memopoisk.lines.add(
                            'Имя:       '+  BOKForm.Source.FieldByName('first_name').AsString
                         );
                         memopoisk.lines.add(
                            'Отчество:  '+  BOKForm.Source.FieldByName('patronymic').AsString
                         );
                         memopoisk.lines.add(
                            'Отдел:     '+  BOKForm.Source.FieldByName('department_show').AsString
                         );
                         memopoisk.lines.add(
                            'Должность: '+  BOKForm.Source.FieldByName('post_show').AsString
                         );
                         memopoisk.lines.add(
                            'Специализация: '+  BOKForm.Source.FieldByName('special_show').AsString
                         );
                         memopoisk.lines.add(
                            'День рождения: '+  BOKForm.Source.FieldByName('birthday').AsString
                         );
                         memopoisk.lines.add( ' '
                         );
                       BOKForm.Source.next;
...
procedure TfmOKAppoint.edStavkaChange(Sender: TObject);
var i : integer;
    is_found : boolean;
begin
  ...
  for i:=1 to CBStavka.Items.Count
  do begin

    if edStavka.text = TData(CBStavka.Items.Objects[ i-1 ]).Id
    then begin
        is_found := true;
        CBStavka.ItemIndex := i-1;
    end;
  ...
  if not is_found
  then begin
    CBStavka.ItemIndex := -1;
  end;

end;

procedure TfmOKAppoint.CBStavkaChange(Sender: TObject);
var d: tdata;
//var    PostCode, DepCode, SalaryFact: string;
//       v: variant;
begin
  ...
  if (CBstavka.ItemIndex<>-1)
  then
    edstavka.text := TData(CBstavka.Items.Objects[CBstavka.ItemIndex]).Id;
