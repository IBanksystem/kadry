...
type
  TBOKOrderForm = class(TBaseForm)
    ...
    procedure CreateRepEDB(PP : Boolean);
...
var
  ...
  rep_branch_,SqlText, fmOKGrid_Mode_Caption : String;
  ListItem: TListItem;
  F                                 : TextFile;
  D: TData;
...
function TBOKOrderForm.GetSQLResultPar(SQLText:String; Par1:String = ''; Par2:String = ''; Par3:String = ''; Par4:String = ''; Par5:String = ''; Par6:String = ''; Par7:String = ''; Par8:String = ''; Par9:String = '') : Variant;
var
  SQLParams:TParams;
begin
...
function TBOKOrderForm.ExecSQLPar(SQLText:String; Par1:String = ''; Par2:String = ''; Par3:String = ''; Par4:String = ''; Par5:String = ''; Par6:String = '') : Integer;
var
  SQLParams:TParams;
begin
  SQLParams:=TParams.Create;
...
procedure TBOKOrderForm.PrepareQueryEDB;
var
  from_or_comma : String;
  I, L : Integer;
begin

// создаем sql-текст запроса
  SqlText := 'Select ';
...
Procedure TBOKOrderForm.CreateRepEDB(PP : Boolean);
Var
...
Begin
      ...
      System.Writeln(f,'');
      System.Writeln(f,'     '+rep_branch_+'  '+CDS_DBBranch.fieldbyname('BANK_NAME').asstring);
      System.Writeln(f,'');
...
////////////////////////////////////////////////////////////
      ...
      System.Writeln(f,'');
      System.Writeln(f,'     '+rep_branch_+'     Отчет сформирован: ' + DateToStr(Now));
      System.Writeln(f,'');
...
// i, col,row,nn,col1:integer;
 ...
 PP:boolean;
 I1,I2,  J2 : Integer;
 cnt,cnt_fil_ : Integer;
 user_name_,Str1 , old_schema: String;
...
////////////////////////////////
  // бегаем по филиалам, выполняем лоад-дата, записываем результат в один и тот же файл
  ...
  Gauge1.MaxValue := CDS_DBBranch.RecordCount;
  cnt_fil_ := 0;
  cnt_fil_ := cnt_fil_+1;
  Gauge1.Progress := cnt_fil_;
...
// без фильтра
    ...
    CDS_DBBranch.filtered := true;
      ...
      do begin
        ...
        then begin
          ...
          User_Name_  :=CDS_DBBranch.fieldbyname('USER_NAME').asstring;
          // перейти в филиал info.init
          cnt:=GetSqlResultpar('select status '+
               ' from '+User_Name_+'.STATUSOPERDAY '+
               ' where branch= :p_1 and id= :p_2 ',
                rep_branch_,'10');
          if cnt=1
          then begin
              memo2.lines.add(chr(149)+' идет процесс закрытия(открытия) дня в филиале '+rep_branch_);
          end else begin
                ExecSQL('alter session set current_schema='+User_Name_);
            ...
            on E: Exception do
              Begin
                MemoProt.lines.add('(МФО '+rep_branch_+')Ошибка. Такой отчет невозможно сгенерировать.');
                MemoProt.lines.add(E.Message);
                MemoProt.lines.add(SqlText);
              End;
            End;
            Gauge1.Progress := 0;
  ...
  ////////////////////////////////
            inc(row);
            V.Cells[row, 2 ].Value := rep_branch_;
  ...
  ////////////////////////////////
    ...
    // вернуться на едб
              ExecSQL('alter session set current_schema='+old_schema);
              ExecSQL('begin info.init;end;');
              ExecSQL('commit');
...
var User_Name_,old_schema : string;
    cnt, cnt_fil_ : integer;
begin
...
//  then actEditQueryExecute(nil);
  ...
  PrepareQueryEDB; // создается sql-текст запроса
...
//    System.Writeln(f,'     '+BOKOrderForm.cds_ss_ok_Pers_Query.FieldByname('NAME').AsString);
    ...
    do begin
      ...
      then begin
        ...
        then begin
          ...
          // сохранить в открытый файл
          CreateRepEDB(CheckBox1.Checked);
        end;
  ...
  //  закрыть файл и вывести в блокнот //
    Finally
      System.CloseFile(f);
    // вернуться на едб
              ExecSQL('alter session set current_schema='+old_schema);
